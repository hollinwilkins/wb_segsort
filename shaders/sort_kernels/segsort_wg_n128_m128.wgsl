...
fn segsort_wg_n128_m128(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_105 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_106 = seg_base + (local_tid ^ 127u); let tmp_107 = smem_keys[tmp_106 * WPT + 0u]; let tmp_108 = smem_vals[tmp_106 * WPT + 0u]; let tmp_109 = keys[0] < tmp_107 || (keys[0] == tmp_107 && values[0] < tmp_108); if tmp_105 == tmp_109 { keys[0] = tmp_107; values[0] = tmp_108; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_110 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_111 = seg_base + (local_tid ^ 32u); let tmp_112 = smem_keys[tmp_111 * WPT + 0u]; let tmp_113 = smem_vals[tmp_111 * WPT + 0u]; let tmp_114 = keys[0] < tmp_112 || (keys[0] == tmp_112 && values[0] < tmp_113); if tmp_110 == tmp_114 { keys[0] = tmp_112; values[0] = tmp_113; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_115 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_116 = seg_base + (local_tid ^ 16u); let tmp_117 = smem_keys[tmp_116 * WPT + 0u]; let tmp_118 = smem_vals[tmp_116 * WPT + 0u]; let tmp_119 = keys[0] < tmp_117 || (keys[0] == tmp_117 && values[0] < tmp_118); if tmp_115 == tmp_119 { keys[0] = tmp_117; values[0] = tmp_118; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_120 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_121 = seg_base + (local_tid ^ 8u); let tmp_122 = smem_keys[tmp_121 * WPT + 0u]; let tmp_123 = smem_vals[tmp_121 * WPT + 0u]; let tmp_124 = keys[0] < tmp_122 || (keys[0] == tmp_122 && values[0] < tmp_123); if tmp_120 == tmp_124 { keys[0] = tmp_122; values[0] = tmp_123; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_125 = extractBits(local_tid, 2u, 1u) != 0u; let tmp_126 = seg_base + (local_tid ^ 4u); let tmp_127 = smem_keys[tmp_126 * WPT + 0u]; let tmp_128 = smem_vals[tmp_126 * WPT + 0u]; let tmp_129 = keys[0] < tmp_127 || (keys[0] == tmp_127 && values[0] < tmp_128); if tmp_125 == tmp_129 { keys[0] = tmp_127; values[0] = tmp_128; } workgroupBarrier(); }
    // exch_paral(tmask:2,swbit:1,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_130 = extractBits(local_tid, 1u, 1u) != 0u; let tmp_131 = seg_base + (local_tid ^ 2u); let tmp_132 = smem_keys[tmp_131 * WPT + 0u]; let tmp_133 = smem_vals[tmp_131 * WPT + 0u]; let tmp_134 = keys[0] < tmp_132 || (keys[0] == tmp_132 && values[0] < tmp_133); if tmp_130 == tmp_134 { keys[0] = tmp_132; values[0] = tmp_133; } workgroupBarrier(); }
    // exch_paral(tmask:1,swbit:0,wpt:1) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; workgroupBarrier(); let tmp_135 = extractBits(local_tid, 0u, 1u) != 0u; let tmp_136 = seg_base + (local_tid ^ 1u); let tmp_137 = smem_keys[tmp_136 * WPT + 0u]; let tmp_138 = smem_vals[tmp_136 * WPT + 0u]; let tmp_139 = keys[0] < tmp_137 || (keys[0] == tmp_137 && values[0] < tmp_138); if tmp_135 == tmp_139 { keys[0] = tmp_137; values[0] = tmp_138; } workgroupBarrier(); }
