...
fn segsort_hybrid_sg8_n1024_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_339 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_340 = seg_base + (local_tid ^ 127u); let tmp_341 = smem_keys[tmp_340 * WPT + 3u]; let tmp_342 = smem_vals[tmp_340 * WPT + 3u]; let tmp_343 = keys[0] < tmp_341 || (keys[0] == tmp_341 && values[0] < tmp_342); if tmp_339 == tmp_343 { keys[0] = tmp_341; values[0] = tmp_342; } let tmp_344 = smem_keys[tmp_340 * WPT + 2u]; let tmp_345 = smem_vals[tmp_340 * WPT + 2u]; let tmp_346 = keys[1] < tmp_344 || (keys[1] == tmp_344 && values[1] < tmp_345); if tmp_339 == tmp_346 { keys[1] = tmp_344; values[1] = tmp_345; } let tmp_347 = smem_keys[tmp_340 * WPT + 1u]; let tmp_348 = smem_vals[tmp_340 * WPT + 1u]; let tmp_349 = keys[2] < tmp_347 || (keys[2] == tmp_347 && values[2] < tmp_348); if tmp_339 == tmp_349 { keys[2] = tmp_347; values[2] = tmp_348; } let tmp_350 = smem_keys[tmp_340 * WPT + 0u]; let tmp_351 = smem_vals[tmp_340 * WPT + 0u]; let tmp_352 = keys[3] < tmp_350 || (keys[3] == tmp_350 && values[3] < tmp_351); if tmp_339 == tmp_352 { keys[3] = tmp_350; values[3] = tmp_351; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_353 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_354 = seg_base + (local_tid ^ 32u); let tmp_355 = smem_keys[tmp_354 * WPT + 0u]; let tmp_356 = smem_vals[tmp_354 * WPT + 0u]; let tmp_357 = keys[0] < tmp_355 || (keys[0] == tmp_355 && values[0] < tmp_356); if tmp_353 == tmp_357 { keys[0] = tmp_355; values[0] = tmp_356; } let tmp_358 = smem_keys[tmp_354 * WPT + 1u]; let tmp_359 = smem_vals[tmp_354 * WPT + 1u]; let tmp_360 = keys[1] < tmp_358 || (keys[1] == tmp_358 && values[1] < tmp_359); if tmp_353 == tmp_360 { keys[1] = tmp_358; values[1] = tmp_359; } let tmp_361 = smem_keys[tmp_354 * WPT + 2u]; let tmp_362 = smem_vals[tmp_354 * WPT + 2u]; let tmp_363 = keys[2] < tmp_361 || (keys[2] == tmp_361 && values[2] < tmp_362); if tmp_353 == tmp_363 { keys[2] = tmp_361; values[2] = tmp_362; } let tmp_364 = smem_keys[tmp_354 * WPT + 3u]; let tmp_365 = smem_vals[tmp_354 * WPT + 3u]; let tmp_366 = keys[3] < tmp_364 || (keys[3] == tmp_364 && values[3] < tmp_365); if tmp_353 == tmp_366 { keys[3] = tmp_364; values[3] = tmp_365; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_367 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_368 = seg_base + (local_tid ^ 16u); let tmp_369 = smem_keys[tmp_368 * WPT + 0u]; let tmp_370 = smem_vals[tmp_368 * WPT + 0u]; let tmp_371 = keys[0] < tmp_369 || (keys[0] == tmp_369 && values[0] < tmp_370); if tmp_367 == tmp_371 { keys[0] = tmp_369; values[0] = tmp_370; } let tmp_372 = smem_keys[tmp_368 * WPT + 1u]; let tmp_373 = smem_vals[tmp_368 * WPT + 1u]; let tmp_374 = keys[1] < tmp_372 || (keys[1] == tmp_372 && values[1] < tmp_373); if tmp_367 == tmp_374 { keys[1] = tmp_372; values[1] = tmp_373; } let tmp_375 = smem_keys[tmp_368 * WPT + 2u]; let tmp_376 = smem_vals[tmp_368 * WPT + 2u]; let tmp_377 = keys[2] < tmp_375 || (keys[2] == tmp_375 && values[2] < tmp_376); if tmp_367 == tmp_377 { keys[2] = tmp_375; values[2] = tmp_376; } let tmp_378 = smem_keys[tmp_368 * WPT + 3u]; let tmp_379 = smem_vals[tmp_368 * WPT + 3u]; let tmp_380 = keys[3] < tmp_378 || (keys[3] == tmp_378 && values[3] < tmp_379); if tmp_367 == tmp_380 { keys[3] = tmp_378; values[3] = tmp_379; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_381 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_382 = seg_base + (local_tid ^ 8u); let tmp_383 = smem_keys[tmp_382 * WPT + 0u]; let tmp_384 = smem_vals[tmp_382 * WPT + 0u]; let tmp_385 = keys[0] < tmp_383 || (keys[0] == tmp_383 && values[0] < tmp_384); if tmp_381 == tmp_385 { keys[0] = tmp_383; values[0] = tmp_384; } let tmp_386 = smem_keys[tmp_382 * WPT + 1u]; let tmp_387 = smem_vals[tmp_382 * WPT + 1u]; let tmp_388 = keys[1] < tmp_386 || (keys[1] == tmp_386 && values[1] < tmp_387); if tmp_381 == tmp_388 { keys[1] = tmp_386; values[1] = tmp_387; } let tmp_389 = smem_keys[tmp_382 * WPT + 2u]; let tmp_390 = smem_vals[tmp_382 * WPT + 2u]; let tmp_391 = keys[2] < tmp_389 || (keys[2] == tmp_389 && values[2] < tmp_390); if tmp_381 == tmp_391 { keys[2] = tmp_389; values[2] = tmp_390; } let tmp_392 = smem_keys[tmp_382 * WPT + 3u]; let tmp_393 = smem_vals[tmp_382 * WPT + 3u]; let tmp_394 = keys[3] < tmp_392 || (keys[3] == tmp_392 && values[3] < tmp_393); if tmp_381 == tmp_394 { keys[3] = tmp_392; values[3] = tmp_393; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:4) 
    {
    ...
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_434 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_434;let tmp_435 = values[0]; values[0] = values[2]; values[2] = tmp_435; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_436 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_436;let tmp_437 = values[1]; values[1] = values[3]; values[3] = tmp_437; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_438 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_438;let tmp_439 = values[0]; values[0] = values[1]; values[1] = tmp_439; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_440 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_440;let tmp_441 = values[2]; values[2] = values[3]; values[3] = tmp_441; }
    }
    // exch_intxn(tmask:255,swbit:7,wpt:4)
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_442 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_443 = seg_base + (local_tid ^ 255u); let tmp_444 = smem_keys[tmp_443 * WPT + 3u]; let tmp_445 = smem_vals[tmp_443 * WPT + 3u]; let tmp_446 = keys[0] < tmp_444 || (keys[0] == tmp_444 && values[0] < tmp_445); if tmp_442 == tmp_446 { keys[0] = tmp_444; values[0] = tmp_445; } let tmp_447 = smem_keys[tmp_443 * WPT + 2u]; let tmp_448 = smem_vals[tmp_443 * WPT + 2u]; let tmp_449 = keys[1] < tmp_447 || (keys[1] == tmp_447 && values[1] < tmp_448); if tmp_442 == tmp_449 { keys[1] = tmp_447; values[1] = tmp_448; } let tmp_450 = smem_keys[tmp_443 * WPT + 1u]; let tmp_451 = smem_vals[tmp_443 * WPT + 1u]; let tmp_452 = keys[2] < tmp_450 || (keys[2] == tmp_450 && values[2] < tmp_451); if tmp_442 == tmp_452 { keys[2] = tmp_450; values[2] = tmp_451; } let tmp_453 = smem_keys[tmp_443 * WPT + 0u]; let tmp_454 = smem_vals[tmp_443 * WPT + 0u]; let tmp_455 = keys[3] < tmp_453 || (keys[3] == tmp_453 && values[3] < tmp_454); if tmp_442 == tmp_455 { keys[3] = tmp_453; values[3] = tmp_454; } workgroupBarrier(); }
    // exch_paral(tmask:64,swbit:6,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_456 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_457 = seg_base + (local_tid ^ 64u); let tmp_458 = smem_keys[tmp_457 * WPT + 0u]; let tmp_459 = smem_vals[tmp_457 * WPT + 0u]; let tmp_460 = keys[0] < tmp_458 || (keys[0] == tmp_458 && values[0] < tmp_459); if tmp_456 == tmp_460 { keys[0] = tmp_458; values[0] = tmp_459; } let tmp_461 = smem_keys[tmp_457 * WPT + 1u]; let tmp_462 = smem_vals[tmp_457 * WPT + 1u]; let tmp_463 = keys[1] < tmp_461 || (keys[1] == tmp_461 && values[1] < tmp_462); if tmp_456 == tmp_463 { keys[1] = tmp_461; values[1] = tmp_462; } let tmp_464 = smem_keys[tmp_457 * WPT + 2u]; let tmp_465 = smem_vals[tmp_457 * WPT + 2u]; let tmp_466 = keys[2] < tmp_464 || (keys[2] == tmp_464 && values[2] < tmp_465); if tmp_456 == tmp_466 { keys[2] = tmp_464; values[2] = tmp_465; } let tmp_467 = smem_keys[tmp_457 * WPT + 3u]; let tmp_468 = smem_vals[tmp_457 * WPT + 3u]; let tmp_469 = keys[3] < tmp_467 || (keys[3] == tmp_467 && values[3] < tmp_468); if tmp_456 == tmp_469 { keys[3] = tmp_467; values[3] = tmp_468; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_470 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_471 = seg_base + (local_tid ^ 32u); let tmp_472 = smem_keys[tmp_471 * WPT + 0u]; let tmp_473 = smem_vals[tmp_471 * WPT + 0u]; let tmp_474 = keys[0] < tmp_472 || (keys[0] == tmp_472 && values[0] < tmp_473); if tmp_470 == tmp_474 { keys[0] = tmp_472; values[0] = tmp_473; } let tmp_475 = smem_keys[tmp_471 * WPT + 1u]; let tmp_476 = smem_vals[tmp_471 * WPT + 1u]; let tmp_477 = keys[1] < tmp_475 || (keys[1] == tmp_475 && values[1] < tmp_476); if tmp_470 == tmp_477 { keys[1] = tmp_475; values[1] = tmp_476; } let tmp_478 = smem_keys[tmp_471 * WPT + 2u]; let tmp_479 = smem_vals[tmp_471 * WPT + 2u]; let tmp_480 = keys[2] < tmp_478 || (keys[2] == tmp_478 && values[2] < tmp_479); if tmp_470 == tmp_480 { keys[2] = tmp_478; values[2] = tmp_479; } let tmp_481 = smem_keys[tmp_471 * WPT + 3u]; let tmp_482 = smem_vals[tmp_471 * WPT + 3u]; let tmp_483 = keys[3] < tmp_481 || (keys[3] == tmp_481 && values[3] < tmp_482); if tmp_470 == tmp_483 { keys[3] = tmp_481; values[3] = tmp_482; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_484 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_485 = seg_base + (local_tid ^ 16u); let tmp_486 = smem_keys[tmp_485 * WPT + 0u]; let tmp_487 = smem_vals[tmp_485 * WPT + 0u]; let tmp_488 = keys[0] < tmp_486 || (keys[0] == tmp_486 && values[0] < tmp_487); if tmp_484 == tmp_488 { keys[0] = tmp_486; values[0] = tmp_487; } let tmp_489 = smem_keys[tmp_485 * WPT + 1u]; let tmp_490 = smem_vals[tmp_485 * WPT + 1u]; let tmp_491 = keys[1] < tmp_489 || (keys[1] == tmp_489 && values[1] < tmp_490); if tmp_484 == tmp_491 { keys[1] = tmp_489; values[1] = tmp_490; } let tmp_492 = smem_keys[tmp_485 * WPT + 2u]; let tmp_493 = smem_vals[tmp_485 * WPT + 2u]; let tmp_494 = keys[2] < tmp_492 || (keys[2] == tmp_492 && values[2] < tmp_493); if tmp_484 == tmp_494 { keys[2] = tmp_492; values[2] = tmp_493; } let tmp_495 = smem_keys[tmp_485 * WPT + 3u]; let tmp_496 = smem_vals[tmp_485 * WPT + 3u]; let tmp_497 = keys[3] < tmp_495 || (keys[3] == tmp_495 && values[3] < tmp_496); if tmp_484 == tmp_497 { keys[3] = tmp_495; values[3] = tmp_496; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_498 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_499 = seg_base + (local_tid ^ 8u); let tmp_500 = smem_keys[tmp_499 * WPT + 0u]; let tmp_501 = smem_vals[tmp_499 * WPT + 0u]; let tmp_502 = keys[0] < tmp_500 || (keys[0] == tmp_500 && values[0] < tmp_501); if tmp_498 == tmp_502 { keys[0] = tmp_500; values[0] = tmp_501; } let tmp_503 = smem_keys[tmp_499 * WPT + 1u]; let tmp_504 = smem_vals[tmp_499 * WPT + 1u]; let tmp_505 = keys[1] < tmp_503 || (keys[1] == tmp_503 && values[1] < tmp_504); if tmp_498 == tmp_505 { keys[1] = tmp_503; values[1] = tmp_504; } let tmp_506 = smem_keys[tmp_499 * WPT + 2u]; let tmp_507 = smem_vals[tmp_499 * WPT + 2u]; let tmp_508 = keys[2] < tmp_506 || (keys[2] == tmp_506 && values[2] < tmp_507); if tmp_498 == tmp_508 { keys[2] = tmp_506; values[2] = tmp_507; } let tmp_509 = smem_keys[tmp_499 * WPT + 3u]; let tmp_510 = smem_vals[tmp_499 * WPT + 3u]; let tmp_511 = keys[3] < tmp_509 || (keys[3] == tmp_509 && values[3] < tmp_510); if tmp_498 == tmp_511 { keys[3] = tmp_509; values[3] = tmp_510; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:4) 
    {
    ...
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_551 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_551;let tmp_552 = values[0]; values[0] = values[2]; values[2] = tmp_552; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_553 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_553;let tmp_554 = values[1]; values[1] = values[3]; values[3] = tmp_554; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_555 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_555;let tmp_556 = values[0]; values[0] = values[1]; values[1] = tmp_556; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_557 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_557;let tmp_558 = values[2]; values[2] = values[3]; values[3] = tmp_558; }
    }
