...
fn segsort_hybrid_sg32_n2048_m64_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; smem_keys[tid_g * WPT + 8u] = keys[8]; smem_vals[tid_g * WPT + 8u] = values[8]; smem_keys[tid_g * WPT + 9u] = keys[9]; smem_vals[tid_g * WPT + 9u] = values[9]; smem_keys[tid_g * WPT + 10u] = keys[10]; smem_vals[tid_g * WPT + 10u] = values[10]; smem_keys[tid_g * WPT + 11u] = keys[11]; smem_vals[tid_g * WPT + 11u] = values[11]; smem_keys[tid_g * WPT + 12u] = keys[12]; smem_vals[tid_g * WPT + 12u] = values[12]; smem_keys[tid_g * WPT + 13u] = keys[13]; smem_vals[tid_g * WPT + 13u] = values[13]; smem_keys[tid_g * WPT + 14u] = keys[14]; smem_vals[tid_g * WPT + 14u] = values[14]; smem_keys[tid_g * WPT + 15u] = keys[15]; smem_vals[tid_g * WPT + 15u] = values[15]; smem_keys[tid_g * WPT + 16u] = keys[16]; smem_vals[tid_g * WPT + 16u] = values[16]; smem_keys[tid_g * WPT + 17u] = keys[17]; smem_vals[tid_g * WPT + 17u] = values[17]; smem_keys[tid_g * WPT + 18u] = keys[18]; smem_vals[tid_g * WPT + 18u] = values[18]; smem_keys[tid_g * WPT + 19u] = keys[19]; smem_vals[tid_g * WPT + 19u] = values[19]; smem_keys[tid_g * WPT + 20u] = keys[20]; smem_vals[tid_g * WPT + 20u] = values[20]; smem_keys[tid_g * WPT + 21u] = keys[21]; smem_vals[tid_g * WPT + 21u] = values[21]; smem_keys[tid_g * WPT + 22u] = keys[22]; smem_vals[tid_g * WPT + 22u] = values[22]; smem_keys[tid_g * WPT + 23u] = keys[23]; smem_vals[tid_g * WPT + 23u] = values[23]; smem_keys[tid_g * WPT + 24u] = keys[24]; smem_vals[tid_g * WPT + 24u] = values[24]; smem_keys[tid_g * WPT + 25u] = keys[25]; smem_vals[tid_g * WPT + 25u] = values[25]; smem_keys[tid_g * WPT + 26u] = keys[26]; smem_vals[tid_g * WPT + 26u] = values[26]; smem_keys[tid_g * WPT + 27u] = keys[27]; smem_vals[tid_g * WPT + 27u] = values[27]; smem_keys[tid_g * WPT + 28u] = keys[28]; smem_vals[tid_g * WPT + 28u] = values[28]; smem_keys[tid_g * WPT + 29u] = keys[29]; smem_vals[tid_g * WPT + 29u] = values[29]; smem_keys[tid_g * WPT + 30u] = keys[30]; smem_vals[tid_g * WPT + 30u] = values[30]; smem_keys[tid_g * WPT + 31u] = keys[31]; smem_vals[tid_g * WPT + 31u] = values[31]; workgroupBarrier(); let tmp_2735 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_2736 = seg_base + (local_tid ^ 63u); let tmp_2737 = smem_keys[tmp_2736 * WPT + 31u]; let tmp_2738 = smem_vals[tmp_2736 * WPT + 31u]; let tmp_2739 = keys[0] < tmp_2737 || (keys[0] == tmp_2737 && values[0] < tmp_2738); if tmp_2735 == tmp_2739 { keys[0] = tmp_2737; values[0] = tmp_2738; } let tmp_2740 = smem_keys[tmp_2736 * WPT + 30u]; let tmp_2741 = smem_vals[tmp_2736 * WPT + 30u]; let tmp_2742 = keys[1] < tmp_2740 || (keys[1] == tmp_2740 && values[1] < tmp_2741); if tmp_2735 == tmp_2742 { keys[1] = tmp_2740; values[1] = tmp_2741; } let tmp_2743 = smem_keys[tmp_2736 * WPT + 29u]; let tmp_2744 = smem_vals[tmp_2736 * WPT + 29u]; let tmp_2745 = keys[2] < tmp_2743 || (keys[2] == tmp_2743 && values[2] < tmp_2744); if tmp_2735 == tmp_2745 { keys[2] = tmp_2743; values[2] = tmp_2744; } let tmp_2746 = smem_keys[tmp_2736 * WPT + 28u]; let tmp_2747 = smem_vals[tmp_2736 * WPT + 28u]; let tmp_2748 = keys[3] < tmp_2746 || (keys[3] == tmp_2746 && values[3] < tmp_2747); if tmp_2735 == tmp_2748 { keys[3] = tmp_2746; values[3] = tmp_2747; } let tmp_2749 = smem_keys[tmp_2736 * WPT + 27u]; let tmp_2750 = smem_vals[tmp_2736 * WPT + 27u]; let tmp_2751 = keys[4] < tmp_2749 || (keys[4] == tmp_2749 && values[4] < tmp_2750); if tmp_2735 == tmp_2751 { keys[4] = tmp_2749; values[4] = tmp_2750; } let tmp_2752 = smem_keys[tmp_2736 * WPT + 26u]; let tmp_2753 = smem_vals[tmp_2736 * WPT + 26u]; let tmp_2754 = keys[5] < tmp_2752 || (keys[5] == tmp_2752 && values[5] < tmp_2753); if tmp_2735 == tmp_2754 { keys[5] = tmp_2752; values[5] = tmp_2753; } let tmp_2755 = smem_keys[tmp_2736 * WPT + 25u]; let tmp_2756 = smem_vals[tmp_2736 * WPT + 25u]; let tmp_2757 = keys[6] < tmp_2755 || (keys[6] == tmp_2755 && values[6] < tmp_2756); if tmp_2735 == tmp_2757 { keys[6] = tmp_2755; values[6] = tmp_2756; } let tmp_2758 = smem_keys[tmp_2736 * WPT + 24u]; let tmp_2759 = smem_vals[tmp_2736 * WPT + 24u]; let tmp_2760 = keys[7] < tmp_2758 || (keys[7] == tmp_2758 && values[7] < tmp_2759); if tmp_2735 == tmp_2760 { keys[7] = tmp_2758; values[7] = tmp_2759; } let tmp_2761 = smem_keys[tmp_2736 * WPT + 23u]; let tmp_2762 = smem_vals[tmp_2736 * WPT + 23u]; let tmp_2763 = keys[8] < tmp_2761 || (keys[8] == tmp_2761 && values[8] < tmp_2762); if tmp_2735 == tmp_2763 { keys[8] = tmp_2761; values[8] = tmp_2762; } let tmp_2764 = smem_keys[tmp_2736 * WPT + 22u]; let tmp_2765 = smem_vals[tmp_2736 * WPT + 22u]; let tmp_2766 = keys[9] < tmp_2764 || (keys[9] == tmp_2764 && values[9] < tmp_2765); if tmp_2735 == tmp_2766 { keys[9] = tmp_2764; values[9] = tmp_2765; } let tmp_2767 = smem_keys[tmp_2736 * WPT + 21u]; let tmp_2768 = smem_vals[tmp_2736 * WPT + 21u]; let tmp_2769 = keys[10] < tmp_2767 || (keys[10] == tmp_2767 && values[10] < tmp_2768); if tmp_2735 == tmp_2769 { keys[10] = tmp_2767; values[10] = tmp_2768; } let tmp_2770 = smem_keys[tmp_2736 * WPT + 20u]; let tmp_2771 = smem_vals[tmp_2736 * WPT + 20u]; let tmp_2772 = keys[11] < tmp_2770 || (keys[11] == tmp_2770 && values[11] < tmp_2771); if tmp_2735 == tmp_2772 { keys[11] = tmp_2770; values[11] = tmp_2771; } let tmp_2773 = smem_keys[tmp_2736 * WPT + 19u]; let tmp_2774 = smem_vals[tmp_2736 * WPT + 19u]; let tmp_2775 = keys[12] < tmp_2773 || (keys[12] == tmp_2773 && values[12] < tmp_2774); if tmp_2735 == tmp_2775 { keys[12] = tmp_2773; values[12] = tmp_2774; } let tmp_2776 = smem_keys[tmp_2736 * WPT + 18u]; let tmp_2777 = smem_vals[tmp_2736 * WPT + 18u]; let tmp_2778 = keys[13] < tmp_2776 || (keys[13] == tmp_2776 && values[13] < tmp_2777); if tmp_2735 == tmp_2778 { keys[13] = tmp_2776; values[13] = tmp_2777; } let tmp_2779 = smem_keys[tmp_2736 * WPT + 17u]; let tmp_2780 = smem_vals[tmp_2736 * WPT + 17u]; let tmp_2781 = keys[14] < tmp_2779 || (keys[14] == tmp_2779 && values[14] < tmp_2780); if tmp_2735 == tmp_2781 { keys[14] = tmp_2779; values[14] = tmp_2780; } let tmp_2782 = smem_keys[tmp_2736 * WPT + 16u]; let tmp_2783 = smem_vals[tmp_2736 * WPT + 16u]; let tmp_2784 = keys[15] < tmp_2782 || (keys[15] == tmp_2782 && values[15] < tmp_2783); if tmp_2735 == tmp_2784 { keys[15] = tmp_2782; values[15] = tmp_2783; } let tmp_2785 = smem_keys[tmp_2736 * WPT + 15u]; let tmp_2786 = smem_vals[tmp_2736 * WPT + 15u]; let tmp_2787 = keys[16] < tmp_2785 || (keys[16] == tmp_2785 && values[16] < tmp_2786); if tmp_2735 == tmp_2787 { keys[16] = tmp_2785; values[16] = tmp_2786; } let tmp_2788 = smem_keys[tmp_2736 * WPT + 14u]; let tmp_2789 = smem_vals[tmp_2736 * WPT + 14u]; let tmp_2790 = keys[17] < tmp_2788 || (keys[17] == tmp_2788 && values[17] < tmp_2789); if tmp_2735 == tmp_2790 { keys[17] = tmp_2788; values[17] = tmp_2789; } let tmp_2791 = smem_keys[tmp_2736 * WPT + 13u]; let tmp_2792 = smem_vals[tmp_2736 * WPT + 13u]; let tmp_2793 = keys[18] < tmp_2791 || (keys[18] == tmp_2791 && values[18] < tmp_2792); if tmp_2735 == tmp_2793 { keys[18] = tmp_2791; values[18] = tmp_2792; } let tmp_2794 = smem_keys[tmp_2736 * WPT + 12u]; let tmp_2795 = smem_vals[tmp_2736 * WPT + 12u]; let tmp_2796 = keys[19] < tmp_2794 || (keys[19] == tmp_2794 && values[19] < tmp_2795); if tmp_2735 == tmp_2796 { keys[19] = tmp_2794; values[19] = tmp_2795; } let tmp_2797 = smem_keys[tmp_2736 * WPT + 11u]; let tmp_2798 = smem_vals[tmp_2736 * WPT + 11u]; let tmp_2799 = keys[20] < tmp_2797 || (keys[20] == tmp_2797 && values[20] < tmp_2798); if tmp_2735 == tmp_2799 { keys[20] = tmp_2797; values[20] = tmp_2798; } let tmp_2800 = smem_keys[tmp_2736 * WPT + 10u]; let tmp_2801 = smem_vals[tmp_2736 * WPT + 10u]; let tmp_2802 = keys[21] < tmp_2800 || (keys[21] == tmp_2800 && values[21] < tmp_2801); if tmp_2735 == tmp_2802 { keys[21] = tmp_2800; values[21] = tmp_2801; } let tmp_2803 = smem_keys[tmp_2736 * WPT + 9u]; let tmp_2804 = smem_vals[tmp_2736 * WPT + 9u]; let tmp_2805 = keys[22] < tmp_2803 || (keys[22] == tmp_2803 && values[22] < tmp_2804); if tmp_2735 == tmp_2805 { keys[22] = tmp_2803; values[22] = tmp_2804; } let tmp_2806 = smem_keys[tmp_2736 * WPT + 8u]; let tmp_2807 = smem_vals[tmp_2736 * WPT + 8u]; let tmp_2808 = keys[23] < tmp_2806 || (keys[23] == tmp_2806 && values[23] < tmp_2807); if tmp_2735 == tmp_2808 { keys[23] = tmp_2806; values[23] = tmp_2807; } let tmp_2809 = smem_keys[tmp_2736 * WPT + 7u]; let tmp_2810 = smem_vals[tmp_2736 * WPT + 7u]; let tmp_2811 = keys[24] < tmp_2809 || (keys[24] == tmp_2809 && values[24] < tmp_2810); if tmp_2735 == tmp_2811 { keys[24] = tmp_2809; values[24] = tmp_2810; } let tmp_2812 = smem_keys[tmp_2736 * WPT + 6u]; let tmp_2813 = smem_vals[tmp_2736 * WPT + 6u]; let tmp_2814 = keys[25] < tmp_2812 || (keys[25] == tmp_2812 && values[25] < tmp_2813); if tmp_2735 == tmp_2814 { keys[25] = tmp_2812; values[25] = tmp_2813; } let tmp_2815 = smem_keys[tmp_2736 * WPT + 5u]; let tmp_2816 = smem_vals[tmp_2736 * WPT + 5u]; let tmp_2817 = keys[26] < tmp_2815 || (keys[26] == tmp_2815 && values[26] < tmp_2816); if tmp_2735 == tmp_2817 { keys[26] = tmp_2815; values[26] = tmp_2816; } let tmp_2818 = smem_keys[tmp_2736 * WPT + 4u]; let tmp_2819 = smem_vals[tmp_2736 * WPT + 4u]; let tmp_2820 = keys[27] < tmp_2818 || (keys[27] == tmp_2818 && values[27] < tmp_2819); if tmp_2735 == tmp_2820 { keys[27] = tmp_2818; values[27] = tmp_2819; } let tmp_2821 = smem_keys[tmp_2736 * WPT + 3u]; let tmp_2822 = smem_vals[tmp_2736 * WPT + 3u]; let tmp_2823 = keys[28] < tmp_2821 || (keys[28] == tmp_2821 && values[28] < tmp_2822); if tmp_2735 == tmp_2823 { keys[28] = tmp_2821; values[28] = tmp_2822; } let tmp_2824 = smem_keys[tmp_2736 * WPT + 2u]; let tmp_2825 = smem_vals[tmp_2736 * WPT + 2u]; let tmp_2826 = keys[29] < tmp_2824 || (keys[29] == tmp_2824 && values[29] < tmp_2825); if tmp_2735 == tmp_2826 { keys[29] = tmp_2824; values[29] = tmp_2825; } let tmp_2827 = smem_keys[tmp_2736 * WPT + 1u]; let tmp_2828 = smem_vals[tmp_2736 * WPT + 1u]; let tmp_2829 = keys[30] < tmp_2827 || (keys[30] == tmp_2827 && values[30] < tmp_2828); if tmp_2735 == tmp_2829 { keys[30] = tmp_2827; values[30] = tmp_2828; } let tmp_2830 = smem_keys[tmp_2736 * WPT + 0u]; let tmp_2831 = smem_vals[tmp_2736 * WPT + 0u]; let tmp_2832 = keys[31] < tmp_2830 || (keys[31] == tmp_2830 && values[31] < tmp_2831); if tmp_2735 == tmp_2832 { keys[31] = tmp_2830; values[31] = tmp_2831; } workgroupBarrier(); }
    ...
    if keys[0] > keys[16] || (keys[0] == keys[16] && values[0] > values[16]) {
    // swap(0,16) 
    { let tmp_3318 = keys[0]; keys[0] = keys[16]; keys[16] = tmp_3318;let tmp_3319 = values[0]; values[0] = values[16]; values[16] = tmp_3319; }
    }
    // cmp_swap(1,17)
    if keys[1] > keys[17] || (keys[1] == keys[17] && values[1] > values[17]) {
    // swap(1,17) 
    { let tmp_3320 = keys[1]; keys[1] = keys[17]; keys[17] = tmp_3320;let tmp_3321 = values[1]; values[1] = values[17]; values[17] = tmp_3321; }
    }
    // cmp_swap(2,18)
    if keys[2] > keys[18] || (keys[2] == keys[18] && values[2] > values[18]) {
    // swap(2,18) 
    { let tmp_3322 = keys[2]; keys[2] = keys[18]; keys[18] = tmp_3322;let tmp_3323 = values[2]; values[2] = values[18]; values[18] = tmp_3323; }
    }
    // cmp_swap(3,19)
    if keys[3] > keys[19] || (keys[3] == keys[19] && values[3] > values[19]) {
    // swap(3,19) 
    { let tmp_3324 = keys[3]; keys[3] = keys[19]; keys[19] = tmp_3324;let tmp_3325 = values[3]; values[3] = values[19]; values[19] = tmp_3325; }
    }
    // cmp_swap(4,20)
    if keys[4] > keys[20] || (keys[4] == keys[20] && values[4] > values[20]) {
    // swap(4,20) 
    { let tmp_3326 = keys[4]; keys[4] = keys[20]; keys[20] = tmp_3326;let tmp_3327 = values[4]; values[4] = values[20]; values[20] = tmp_3327; }
    }
    // cmp_swap(5,21)
    if keys[5] > keys[21] || (keys[5] == keys[21] && values[5] > values[21]) {
    // swap(5,21) 
    { let tmp_3328 = keys[5]; keys[5] = keys[21]; keys[21] = tmp_3328;let tmp_3329 = values[5]; values[5] = values[21]; values[21] = tmp_3329; }
    }
    // cmp_swap(6,22)
    if keys[6] > keys[22] || (keys[6] == keys[22] && values[6] > values[22]) {
    // swap(6,22) 
    { let tmp_3330 = keys[6]; keys[6] = keys[22]; keys[22] = tmp_3330;let tmp_3331 = values[6]; values[6] = values[22]; values[22] = tmp_3331; }
    }
    // cmp_swap(7,23)
    if keys[7] > keys[23] || (keys[7] == keys[23] && values[7] > values[23]) {
    // swap(7,23) 
    { let tmp_3332 = keys[7]; keys[7] = keys[23]; keys[23] = tmp_3332;let tmp_3333 = values[7]; values[7] = values[23]; values[23] = tmp_3333; }
    }
    // cmp_swap(8,24)
    if keys[8] > keys[24] || (keys[8] == keys[24] && values[8] > values[24]) {
    // swap(8,24) 
    { let tmp_3334 = keys[8]; keys[8] = keys[24]; keys[24] = tmp_3334;let tmp_3335 = values[8]; values[8] = values[24]; values[24] = tmp_3335; }
    }
    // cmp_swap(9,25)
    if keys[9] > keys[25] || (keys[9] == keys[25] && values[9] > values[25]) {
    // swap(9,25) 
    { let tmp_3336 = keys[9]; keys[9] = keys[25]; keys[25] = tmp_3336;let tmp_3337 = values[9]; values[9] = values[25]; values[25] = tmp_3337; }
    }
    // cmp_swap(10,26)
    if keys[10] > keys[26] || (keys[10] == keys[26] && values[10] > values[26]) {
    // swap(10,26) 
    { let tmp_3338 = keys[10]; keys[10] = keys[26]; keys[26] = tmp_3338;let tmp_3339 = values[10]; values[10] = values[26]; values[26] = tmp_3339; }
    }
    // cmp_swap(11,27)
    if keys[11] > keys[27] || (keys[11] == keys[27] && values[11] > values[27]) {
    // swap(11,27) 
    { let tmp_3340 = keys[11]; keys[11] = keys[27]; keys[27] = tmp_3340;let tmp_3341 = values[11]; values[11] = values[27]; values[27] = tmp_3341; }
    }
    // cmp_swap(12,28)
    if keys[12] > keys[28] || (keys[12] == keys[28] && values[12] > values[28]) {
    // swap(12,28) 
    { let tmp_3342 = keys[12]; keys[12] = keys[28]; keys[28] = tmp_3342;let tmp_3343 = values[12]; values[12] = values[28]; values[28] = tmp_3343; }
    }
    // cmp_swap(13,29)
    if keys[13] > keys[29] || (keys[13] == keys[29] && values[13] > values[29]) {
    // swap(13,29) 
    { let tmp_3344 = keys[13]; keys[13] = keys[29]; keys[29] = tmp_3344;let tmp_3345 = values[13]; values[13] = values[29]; values[29] = tmp_3345; }
    }
    // cmp_swap(14,30)
    if keys[14] > keys[30] || (keys[14] == keys[30] && values[14] > values[30]) {
    // swap(14,30) 
    { let tmp_3346 = keys[14]; keys[14] = keys[30]; keys[30] = tmp_3346;let tmp_3347 = values[14]; values[14] = values[30]; values[30] = tmp_3347; }
    }
    // cmp_swap(15,31)
    if keys[15] > keys[31] || (keys[15] == keys[31] && values[15] > values[31]) {
    // swap(15,31) 
    { let tmp_3348 = keys[15]; keys[15] = keys[31]; keys[31] = tmp_3348;let tmp_3349 = values[15]; values[15] = values[31]; values[31] = tmp_3349; }
    }
    // exch_local(8,32) 
    // cmp_swap(0,8)
    if keys[0] > keys[8] || (keys[0] == keys[8] && values[0] > values[8]) {
    // swap(0,8) 
    { let tmp_3350 = keys[0]; keys[0] = keys[8]; keys[8] = tmp_3350;let tmp_3351 = values[0]; values[0] = values[8]; values[8] = tmp_3351; }
    }
    // cmp_swap(1,9)
    if keys[1] > keys[9] || (keys[1] == keys[9] && values[1] > values[9]) {
    // swap(1,9) 
    { let tmp_3352 = keys[1]; keys[1] = keys[9]; keys[9] = tmp_3352;let tmp_3353 = values[1]; values[1] = values[9]; values[9] = tmp_3353; }
    }
    // cmp_swap(2,10)
    if keys[2] > keys[10] || (keys[2] == keys[10] && values[2] > values[10]) {
    // swap(2,10) 
    { let tmp_3354 = keys[2]; keys[2] = keys[10]; keys[10] = tmp_3354;let tmp_3355 = values[2]; values[2] = values[10]; values[10] = tmp_3355; }
    }
    // cmp_swap(3,11)
    if keys[3] > keys[11] || (keys[3] == keys[11] && values[3] > values[11]) {
    // swap(3,11) 
    { let tmp_3356 = keys[3]; keys[3] = keys[11]; keys[11] = tmp_3356;let tmp_3357 = values[3]; values[3] = values[11]; values[11] = tmp_3357; }
    }
    // cmp_swap(4,12)
    if keys[4] > keys[12] || (keys[4] == keys[12] && values[4] > values[12]) {
    // swap(4,12) 
    { let tmp_3358 = keys[4]; keys[4] = keys[12]; keys[12] = tmp_3358;let tmp_3359 = values[4]; values[4] = values[12]; values[12] = tmp_3359; }
    }
    // cmp_swap(5,13)
    if keys[5] > keys[13] || (keys[5] == keys[13] && values[5] > values[13]) {
    // swap(5,13) 
    { let tmp_3360 = keys[5]; keys[5] = keys[13]; keys[13] = tmp_3360;let tmp_3361 = values[5]; values[5] = values[13]; values[13] = tmp_3361; }
    }
    // cmp_swap(6,14)
    if keys[6] > keys[14] || (keys[6] == keys[14] && values[6] > values[14]) {
    // swap(6,14) 
    { let tmp_3362 = keys[6]; keys[6] = keys[14]; keys[14] = tmp_3362;let tmp_3363 = values[6]; values[6] = values[14]; values[14] = tmp_3363; }
    }
    // cmp_swap(7,15)
    if keys[7] > keys[15] || (keys[7] == keys[15] && values[7] > values[15]) {
    // swap(7,15) 
    { let tmp_3364 = keys[7]; keys[7] = keys[15]; keys[15] = tmp_3364;let tmp_3365 = values[7]; values[7] = values[15]; values[15] = tmp_3365; }
    }
    // cmp_swap(16,24)
    if keys[16] > keys[24] || (keys[16] == keys[24] && values[16] > values[24]) {
    // swap(16,24) 
    { let tmp_3366 = keys[16]; keys[16] = keys[24]; keys[24] = tmp_3366;let tmp_3367 = values[16]; values[16] = values[24]; values[24] = tmp_3367; }
    }
    // cmp_swap(17,25)
    if keys[17] > keys[25] || (keys[17] == keys[25] && values[17] > values[25]) {
    // swap(17,25) 
    { let tmp_3368 = keys[17]; keys[17] = keys[25]; keys[25] = tmp_3368;let tmp_3369 = values[17]; values[17] = values[25]; values[25] = tmp_3369; }
    }
    // cmp_swap(18,26)
    if keys[18] > keys[26] || (keys[18] == keys[26] && values[18] > values[26]) {
    // swap(18,26) 
    { let tmp_3370 = keys[18]; keys[18] = keys[26]; keys[26] = tmp_3370;let tmp_3371 = values[18]; values[18] = values[26]; values[26] = tmp_3371; }
    }
    // cmp_swap(19,27)
    if keys[19] > keys[27] || (keys[19] == keys[27] && values[19] > values[27]) {
    // swap(19,27) 
    { let tmp_3372 = keys[19]; keys[19] = keys[27]; keys[27] = tmp_3372;let tmp_3373 = values[19]; values[19] = values[27]; values[27] = tmp_3373; }
    }
    // cmp_swap(20,28)
    if keys[20] > keys[28] || (keys[20] == keys[28] && values[20] > values[28]) {
    // swap(20,28) 
    { let tmp_3374 = keys[20]; keys[20] = keys[28]; keys[28] = tmp_3374;let tmp_3375 = values[20]; values[20] = values[28]; values[28] = tmp_3375; }
    }
    // cmp_swap(21,29)
    if keys[21] > keys[29] || (keys[21] == keys[29] && values[21] > values[29]) {
    // swap(21,29) 
    { let tmp_3376 = keys[21]; keys[21] = keys[29]; keys[29] = tmp_3376;let tmp_3377 = values[21]; values[21] = values[29]; values[29] = tmp_3377; }
    }
    // cmp_swap(22,30)
    if keys[22] > keys[30] || (keys[22] == keys[30] && values[22] > values[30]) {
    // swap(22,30) 
    { let tmp_3378 = keys[22]; keys[22] = keys[30]; keys[30] = tmp_3378;let tmp_3379 = values[22]; values[22] = values[30]; values[30] = tmp_3379; }
    }
    // cmp_swap(23,31)
    if keys[23] > keys[31] || (keys[23] == keys[31] && values[23] > values[31]) {
    // swap(23,31) 
    { let tmp_3380 = keys[23]; keys[23] = keys[31]; keys[31] = tmp_3380;let tmp_3381 = values[23]; values[23] = values[31]; values[31] = tmp_3381; }
    }
    // exch_local(4,32) 
    // cmp_swap(0,4)
    if keys[0] > keys[4] || (keys[0] == keys[4] && values[0] > values[4]) {
    // swap(0,4) 
    { let tmp_3382 = keys[0]; keys[0] = keys[4]; keys[4] = tmp_3382;let tmp_3383 = values[0]; values[0] = values[4]; values[4] = tmp_3383; }
    }
    // cmp_swap(1,5)
    if keys[1] > keys[5] || (keys[1] == keys[5] && values[1] > values[5]) {
    // swap(1,5) 
    { let tmp_3384 = keys[1]; keys[1] = keys[5]; keys[5] = tmp_3384;let tmp_3385 = values[1]; values[1] = values[5]; values[5] = tmp_3385; }
    }
    // cmp_swap(2,6)
    if keys[2] > keys[6] || (keys[2] == keys[6] && values[2] > values[6]) {
    // swap(2,6) 
    { let tmp_3386 = keys[2]; keys[2] = keys[6]; keys[6] = tmp_3386;let tmp_3387 = values[2]; values[2] = values[6]; values[6] = tmp_3387; }
    }
    // cmp_swap(3,7)
    if keys[3] > keys[7] || (keys[3] == keys[7] && values[3] > values[7]) {
    // swap(3,7) 
    { let tmp_3388 = keys[3]; keys[3] = keys[7]; keys[7] = tmp_3388;let tmp_3389 = values[3]; values[3] = values[7]; values[7] = tmp_3389; }
    }
    // cmp_swap(8,12)
    if keys[8] > keys[12] || (keys[8] == keys[12] && values[8] > values[12]) {
    // swap(8,12) 
    { let tmp_3390 = keys[8]; keys[8] = keys[12]; keys[12] = tmp_3390;let tmp_3391 = values[8]; values[8] = values[12]; values[12] = tmp_3391; }
    }
    // cmp_swap(9,13)
    if keys[9] > keys[13] || (keys[9] == keys[13] && values[9] > values[13]) {
    // swap(9,13) 
    { let tmp_3392 = keys[9]; keys[9] = keys[13]; keys[13] = tmp_3392;let tmp_3393 = values[9]; values[9] = values[13]; values[13] = tmp_3393; }
    }
    // cmp_swap(10,14)
    if keys[10] > keys[14] || (keys[10] == keys[14] && values[10] > values[14]) {
    // swap(10,14) 
    { let tmp_3394 = keys[10]; keys[10] = keys[14]; keys[14] = tmp_3394;let tmp_3395 = values[10]; values[10] = values[14]; values[14] = tmp_3395; }
    }
    // cmp_swap(11,15)
    if keys[11] > keys[15] || (keys[11] == keys[15] && values[11] > values[15]) {
    // swap(11,15) 
    { let tmp_3396 = keys[11]; keys[11] = keys[15]; keys[15] = tmp_3396;let tmp_3397 = values[11]; values[11] = values[15]; values[15] = tmp_3397; }
    }
    // cmp_swap(16,20)
    if keys[16] > keys[20] || (keys[16] == keys[20] && values[16] > values[20]) {
    // swap(16,20) 
    { let tmp_3398 = keys[16]; keys[16] = keys[20]; keys[20] = tmp_3398;let tmp_3399 = values[16]; values[16] = values[20]; values[20] = tmp_3399; }
    }
    // cmp_swap(17,21)
    if keys[17] > keys[21] || (keys[17] == keys[21] && values[17] > values[21]) {
    // swap(17,21) 
    { let tmp_3400 = keys[17]; keys[17] = keys[21]; keys[21] = tmp_3400;let tmp_3401 = values[17]; values[17] = values[21]; values[21] = tmp_3401; }
    }
    // cmp_swap(18,22)
    if keys[18] > keys[22] || (keys[18] == keys[22] && values[18] > values[22]) {
    // swap(18,22) 
    { let tmp_3402 = keys[18]; keys[18] = keys[22]; keys[22] = tmp_3402;let tmp_3403 = values[18]; values[18] = values[22]; values[22] = tmp_3403; }
    }
    // cmp_swap(19,23)
    if keys[19] > keys[23] || (keys[19] == keys[23] && values[19] > values[23]) {
    // swap(19,23) 
    { let tmp_3404 = keys[19]; keys[19] = keys[23]; keys[23] = tmp_3404;let tmp_3405 = values[19]; values[19] = values[23]; values[23] = tmp_3405; }
    }
    // cmp_swap(24,28)
    if keys[24] > keys[28] || (keys[24] == keys[28] && values[24] > values[28]) {
    // swap(24,28) 
    { let tmp_3406 = keys[24]; keys[24] = keys[28]; keys[28] = tmp_3406;let tmp_3407 = values[24]; values[24] = values[28]; values[28] = tmp_3407; }
    }
    // cmp_swap(25,29)
    if keys[25] > keys[29] || (keys[25] == keys[29] && values[25] > values[29]) {
    // swap(25,29) 
    { let tmp_3408 = keys[25]; keys[25] = keys[29]; keys[29] = tmp_3408;let tmp_3409 = values[25]; values[25] = values[29]; values[29] = tmp_3409; }
    }
    // cmp_swap(26,30)
    if keys[26] > keys[30] || (keys[26] == keys[30] && values[26] > values[30]) {
    // swap(26,30) 
    { let tmp_3410 = keys[26]; keys[26] = keys[30]; keys[30] = tmp_3410;let tmp_3411 = values[26]; values[26] = values[30]; values[30] = tmp_3411; }
    }
    // cmp_swap(27,31)
    if keys[27] > keys[31] || (keys[27] == keys[31] && values[27] > values[31]) {
    // swap(27,31) 
    { let tmp_3412 = keys[27]; keys[27] = keys[31]; keys[31] = tmp_3412;let tmp_3413 = values[27]; values[27] = values[31]; values[31] = tmp_3413; }
    }
    // exch_local(2,32) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_3414 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_3414;let tmp_3415 = values[0]; values[0] = values[2]; values[2] = tmp_3415; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_3416 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_3416;let tmp_3417 = values[1]; values[1] = values[3]; values[3] = tmp_3417; }
    }
    // cmp_swap(4,6)
    if keys[4] > keys[6] || (keys[4] == keys[6] && values[4] > values[6]) {
    // swap(4,6) 
    { let tmp_3418 = keys[4]; keys[4] = keys[6]; keys[6] = tmp_3418;let tmp_3419 = values[4]; values[4] = values[6]; values[6] = tmp_3419; }
    }
    // cmp_swap(5,7)
    if keys[5] > keys[7] || (keys[5] == keys[7] && values[5] > values[7]) {
    // swap(5,7) 
    { let tmp_3420 = keys[5]; keys[5] = keys[7]; keys[7] = tmp_3420;let tmp_3421 = values[5]; values[5] = values[7]; values[7] = tmp_3421; }
    }
    // cmp_swap(8,10)
    if keys[8] > keys[10] || (keys[8] == keys[10] && values[8] > values[10]) {
    // swap(8,10) 
    { let tmp_3422 = keys[8]; keys[8] = keys[10]; keys[10] = tmp_3422;let tmp_3423 = values[8]; values[8] = values[10]; values[10] = tmp_3423; }
    }
    // cmp_swap(9,11)
    if keys[9] > keys[11] || (keys[9] == keys[11] && values[9] > values[11]) {
    // swap(9,11) 
    { let tmp_3424 = keys[9]; keys[9] = keys[11]; keys[11] = tmp_3424;let tmp_3425 = values[9]; values[9] = values[11]; values[11] = tmp_3425; }
    }
    // cmp_swap(12,14)
    if keys[12] > keys[14] || (keys[12] == keys[14] && values[12] > values[14]) {
    // swap(12,14) 
    { let tmp_3426 = keys[12]; keys[12] = keys[14]; keys[14] = tmp_3426;let tmp_3427 = values[12]; values[12] = values[14]; values[14] = tmp_3427; }
    }
    // cmp_swap(13,15)
    if keys[13] > keys[15] || (keys[13] == keys[15] && values[13] > values[15]) {
    // swap(13,15) 
    { let tmp_3428 = keys[13]; keys[13] = keys[15]; keys[15] = tmp_3428;let tmp_3429 = values[13]; values[13] = values[15]; values[15] = tmp_3429; }
    }
    // cmp_swap(16,18)
    if keys[16] > keys[18] || (keys[16] == keys[18] && values[16] > values[18]) {
    // swap(16,18) 
    { let tmp_3430 = keys[16]; keys[16] = keys[18]; keys[18] = tmp_3430;let tmp_3431 = values[16]; values[16] = values[18]; values[18] = tmp_3431; }
    }
    // cmp_swap(17,19)
    if keys[17] > keys[19] || (keys[17] == keys[19] && values[17] > values[19]) {
    // swap(17,19) 
    { let tmp_3432 = keys[17]; keys[17] = keys[19]; keys[19] = tmp_3432;let tmp_3433 = values[17]; values[17] = values[19]; values[19] = tmp_3433; }
    }
    // cmp_swap(20,22)
    if keys[20] > keys[22] || (keys[20] == keys[22] && values[20] > values[22]) {
    // swap(20,22) 
    { let tmp_3434 = keys[20]; keys[20] = keys[22]; keys[22] = tmp_3434;let tmp_3435 = values[20]; values[20] = values[22]; values[22] = tmp_3435; }
    }
    // cmp_swap(21,23)
    if keys[21] > keys[23] || (keys[21] == keys[23] && values[21] > values[23]) {
    // swap(21,23) 
    { let tmp_3436 = keys[21]; keys[21] = keys[23]; keys[23] = tmp_3436;let tmp_3437 = values[21]; values[21] = values[23]; values[23] = tmp_3437; }
    }
    // cmp_swap(24,26)
    if keys[24] > keys[26] || (keys[24] == keys[26] && values[24] > values[26]) {
    // swap(24,26) 
    { let tmp_3438 = keys[24]; keys[24] = keys[26]; keys[26] = tmp_3438;let tmp_3439 = values[24]; values[24] = values[26]; values[26] = tmp_3439; }
    }
    // cmp_swap(25,27)
    if keys[25] > keys[27] || (keys[25] == keys[27] && values[25] > values[27]) {
    // swap(25,27) 
    { let tmp_3440 = keys[25]; keys[25] = keys[27]; keys[27] = tmp_3440;let tmp_3441 = values[25]; values[25] = values[27]; values[27] = tmp_3441; }
    }
    // cmp_swap(28,30)
    if keys[28] > keys[30] || (keys[28] == keys[30] && values[28] > values[30]) {
    // swap(28,30) 
    { let tmp_3442 = keys[28]; keys[28] = keys[30]; keys[30] = tmp_3442;let tmp_3443 = values[28]; values[28] = values[30]; values[30] = tmp_3443; }
    }
    // cmp_swap(29,31)
    if keys[29] > keys[31] || (keys[29] == keys[31] && values[29] > values[31]) {
    // swap(29,31) 
    { let tmp_3444 = keys[29]; keys[29] = keys[31]; keys[31] = tmp_3444;let tmp_3445 = values[29]; values[29] = values[31]; values[31] = tmp_3445; }
    }
    // exch_local(1,32) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_3446 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_3446;let tmp_3447 = values[0]; values[0] = values[1]; values[1] = tmp_3447; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_3448 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_3448;let tmp_3449 = values[2]; values[2] = values[3]; values[3] = tmp_3449; }
    }
    // cmp_swap(4,5)
    if keys[4] > keys[5] || (keys[4] == keys[5] && values[4] > values[5]) {
    // swap(4,5) 
    { let tmp_3450 = keys[4]; keys[4] = keys[5]; keys[5] = tmp_3450;let tmp_3451 = values[4]; values[4] = values[5]; values[5] = tmp_3451; }
    }
    // cmp_swap(6,7)
    if keys[6] > keys[7] || (keys[6] == keys[7] && values[6] > values[7]) {
    // swap(6,7) 
    { let tmp_3452 = keys[6]; keys[6] = keys[7]; keys[7] = tmp_3452;let tmp_3453 = values[6]; values[6] = values[7]; values[7] = tmp_3453; }
    }
    // cmp_swap(8,9)
    if keys[8] > keys[9] || (keys[8] == keys[9] && values[8] > values[9]) {
    // swap(8,9) 
    { let tmp_3454 = keys[8]; keys[8] = keys[9]; keys[9] = tmp_3454;let tmp_3455 = values[8]; values[8] = values[9]; values[9] = tmp_3455; }
    }
    // cmp_swap(10,11)
    if keys[10] > keys[11] || (keys[10] == keys[11] && values[10] > values[11]) {
    // swap(10,11) 
    { let tmp_3456 = keys[10]; keys[10] = keys[11]; keys[11] = tmp_3456;let tmp_3457 = values[10]; values[10] = values[11]; values[11] = tmp_3457; }
    }
    // cmp_swap(12,13)
    if keys[12] > keys[13] || (keys[12] == keys[13] && values[12] > values[13]) {
    // swap(12,13) 
    { let tmp_3458 = keys[12]; keys[12] = keys[13]; keys[13] = tmp_3458;let tmp_3459 = values[12]; values[12] = values[13]; values[13] = tmp_3459; }
    }
    // cmp_swap(14,15)
    if keys[14] > keys[15] || (keys[14] == keys[15] && values[14] > values[15]) {
    // swap(14,15) 
    { let tmp_3460 = keys[14]; keys[14] = keys[15]; keys[15] = tmp_3460;let tmp_3461 = values[14]; values[14] = values[15]; values[15] = tmp_3461; }
    }
    // cmp_swap(16,17)
    if keys[16] > keys[17] || (keys[16] == keys[17] && values[16] > values[17]) {
    // swap(16,17) 
    { let tmp_3462 = keys[16]; keys[16] = keys[17]; keys[17] = tmp_3462;let tmp_3463 = values[16]; values[16] = values[17]; values[17] = tmp_3463; }
    }
    // cmp_swap(18,19)
    if keys[18] > keys[19] || (keys[18] == keys[19] && values[18] > values[19]) {
    // swap(18,19) 
    { let tmp_3464 = keys[18]; keys[18] = keys[19]; keys[19] = tmp_3464;let tmp_3465 = values[18]; values[18] = values[19]; values[19] = tmp_3465; }
    }
    // cmp_swap(20,21)
    if keys[20] > keys[21] || (keys[20] == keys[21] && values[20] > values[21]) {
    // swap(20,21) 
    { let tmp_3466 = keys[20]; keys[20] = keys[21]; keys[21] = tmp_3466;let tmp_3467 = values[20]; values[20] = values[21]; values[21] = tmp_3467; }
    }
    // cmp_swap(22,23)
    if keys[22] > keys[23] || (keys[22] == keys[23] && values[22] > values[23]) {
    // swap(22,23) 
    { let tmp_3468 = keys[22]; keys[22] = keys[23]; keys[23] = tmp_3468;let tmp_3469 = values[22]; values[22] = values[23]; values[23] = tmp_3469; }
    }
    // cmp_swap(24,25)
    if keys[24] > keys[25] || (keys[24] == keys[25] && values[24] > values[25]) {
    // swap(24,25) 
    { let tmp_3470 = keys[24]; keys[24] = keys[25]; keys[25] = tmp_3470;let tmp_3471 = values[24]; values[24] = values[25]; values[25] = tmp_3471; }
    }
    // cmp_swap(26,27)
    if keys[26] > keys[27] || (keys[26] == keys[27] && values[26] > values[27]) {
    // swap(26,27) 
    { let tmp_3472 = keys[26]; keys[26] = keys[27]; keys[27] = tmp_3472;let tmp_3473 = values[26]; values[26] = values[27]; values[27] = tmp_3473; }
    }
    // cmp_swap(28,29)
    if keys[28] > keys[29] || (keys[28] == keys[29] && values[28] > values[29]) {
    // swap(28,29) 
    { let tmp_3474 = keys[28]; keys[28] = keys[29]; keys[29] = tmp_3474;let tmp_3475 = values[28]; values[28] = values[29]; values[29] = tmp_3475; }
    }
    // cmp_swap(30,31)
    if keys[30] > keys[31] || (keys[30] == keys[31] && values[30] > values[31]) {
    // swap(30,31) 
    { let tmp_3476 = keys[30]; keys[30] = keys[31]; keys[31] = tmp_3476;let tmp_3477 = values[30]; values[30] = values[31]; values[31] = tmp_3477; }
    }
