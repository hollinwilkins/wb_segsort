...
const M: u32 = 128u;
const WPT: u32 = 32u;
const SG: u32 = 16u;      // one CuteSort run spans SG lanes; RUN = SG*WPT = 512
...
fn segsort_cutemerge_sg16_smem32k_n4096_m128_striped(
    ...
    {  // cute run for register slot 8
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 8u * SG + rank] = key_k;
        smem_vals[sub_block + 8u * SG + rank] = values[8];
    }
    {  // cute run for register slot 9
        let key_k = keys[9];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 9u * SG + rank] = key_k;
        smem_vals[sub_block + 9u * SG + rank] = values[9];
    }
    {  // cute run for register slot 10
        let key_k = keys[10];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 10u * SG + rank] = key_k;
        smem_vals[sub_block + 10u * SG + rank] = values[10];
    }
    {  // cute run for register slot 11
        let key_k = keys[11];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 11u * SG + rank] = key_k;
        smem_vals[sub_block + 11u * SG + rank] = values[11];
    }
    {  // cute run for register slot 12
        let key_k = keys[12];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 12u * SG + rank] = key_k;
        smem_vals[sub_block + 12u * SG + rank] = values[12];
    }
    {  // cute run for register slot 13
        let key_k = keys[13];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 13u * SG + rank] = key_k;
        smem_vals[sub_block + 13u * SG + rank] = values[13];
    }
    {  // cute run for register slot 14
        let key_k = keys[14];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 14u * SG + rank] = key_k;
        smem_vals[sub_block + 14u * SG + rank] = values[14];
    }
    {  // cute run for register slot 15
        let key_k = keys[15];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 15u * SG + rank] = key_k;
        smem_vals[sub_block + 15u * SG + rank] = values[15];
    }
    {  // cute run for register slot 16
        let key_k = keys[16];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 16u * SG + rank] = key_k;
        smem_vals[sub_block + 16u * SG + rank] = values[16];
    }
    {  // cute run for register slot 17
        let key_k = keys[17];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 17u * SG + rank] = key_k;
        smem_vals[sub_block + 17u * SG + rank] = values[17];
    }
    {  // cute run for register slot 18
        let key_k = keys[18];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 18u * SG + rank] = key_k;
        smem_vals[sub_block + 18u * SG + rank] = values[18];
    }
    {  // cute run for register slot 19
        let key_k = keys[19];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 19u * SG + rank] = key_k;
        smem_vals[sub_block + 19u * SG + rank] = values[19];
    }
    {  // cute run for register slot 20
        let key_k = keys[20];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 20u * SG + rank] = key_k;
        smem_vals[sub_block + 20u * SG + rank] = values[20];
    }
    {  // cute run for register slot 21
        let key_k = keys[21];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 21u * SG + rank] = key_k;
        smem_vals[sub_block + 21u * SG + rank] = values[21];
    }
    {  // cute run for register slot 22
        let key_k = keys[22];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 22u * SG + rank] = key_k;
        smem_vals[sub_block + 22u * SG + rank] = values[22];
    }
    {  // cute run for register slot 23
        let key_k = keys[23];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 23u * SG + rank] = key_k;
        smem_vals[sub_block + 23u * SG + rank] = values[23];
    }
    {  // cute run for register slot 24
        let key_k = keys[24];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 24u * SG + rank] = key_k;
        smem_vals[sub_block + 24u * SG + rank] = values[24];
    }
    {  // cute run for register slot 25
        let key_k = keys[25];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 25u * SG + rank] = key_k;
        smem_vals[sub_block + 25u * SG + rank] = values[25];
    }
    {  // cute run for register slot 26
        let key_k = keys[26];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 26u * SG + rank] = key_k;
        smem_vals[sub_block + 26u * SG + rank] = values[26];
    }
    {  // cute run for register slot 27
        let key_k = keys[27];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 27u * SG + rank] = key_k;
        smem_vals[sub_block + 27u * SG + rank] = values[27];
    }
    {  // cute run for register slot 28
        let key_k = keys[28];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 28u * SG + rank] = key_k;
        smem_vals[sub_block + 28u * SG + rank] = values[28];
    }
    {  // cute run for register slot 29
        let key_k = keys[29];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 29u * SG + rank] = key_k;
        smem_vals[sub_block + 29u * SG + rank] = values[29];
    }
    {  // cute run for register slot 30
        let key_k = keys[30];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 30u * SG + rank] = key_k;
        smem_vals[sub_block + 30u * SG + rank] = values[30];
    }
    {  // cute run for register slot 31
        let key_k = keys[31];
        var ge_mask = lane_mask_lt(sid);
        for (var bit = 0u; bit < 32u; bit = bit + 1u) {
            let is_zero = (key_k & (1u << bit)) == 0u;
        ...
        let rank = ballot_popc(ge_mask & bin_mask);
        smem_keys[sub_block + 31u * SG + rank] = key_k;
        smem_vals[sub_block + 31u * SG + rank] = values[31];
    }
    workgroupBarrier();
