...
fn segsort_hybrid_sg8_n2048_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_723 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_724 = seg_base + (local_tid ^ 127u); let tmp_725 = smem_keys[tmp_724 * WPT + 7u]; let tmp_726 = smem_vals[tmp_724 * WPT + 7u]; let tmp_727 = keys[0] < tmp_725 || (keys[0] == tmp_725 && values[0] < tmp_726); if tmp_723 == tmp_727 { keys[0] = tmp_725; values[0] = tmp_726; } let tmp_728 = smem_keys[tmp_724 * WPT + 6u]; let tmp_729 = smem_vals[tmp_724 * WPT + 6u]; let tmp_730 = keys[1] < tmp_728 || (keys[1] == tmp_728 && values[1] < tmp_729); if tmp_723 == tmp_730 { keys[1] = tmp_728; values[1] = tmp_729; } let tmp_731 = smem_keys[tmp_724 * WPT + 5u]; let tmp_732 = smem_vals[tmp_724 * WPT + 5u]; let tmp_733 = keys[2] < tmp_731 || (keys[2] == tmp_731 && values[2] < tmp_732); if tmp_723 == tmp_733 { keys[2] = tmp_731; values[2] = tmp_732; } let tmp_734 = smem_keys[tmp_724 * WPT + 4u]; let tmp_735 = smem_vals[tmp_724 * WPT + 4u]; let tmp_736 = keys[3] < tmp_734 || (keys[3] == tmp_734 && values[3] < tmp_735); if tmp_723 == tmp_736 { keys[3] = tmp_734; values[3] = tmp_735; } let tmp_737 = smem_keys[tmp_724 * WPT + 3u]; let tmp_738 = smem_vals[tmp_724 * WPT + 3u]; let tmp_739 = keys[4] < tmp_737 || (keys[4] == tmp_737 && values[4] < tmp_738); if tmp_723 == tmp_739 { keys[4] = tmp_737; values[4] = tmp_738; } let tmp_740 = smem_keys[tmp_724 * WPT + 2u]; let tmp_741 = smem_vals[tmp_724 * WPT + 2u]; let tmp_742 = keys[5] < tmp_740 || (keys[5] == tmp_740 && values[5] < tmp_741); if tmp_723 == tmp_742 { keys[5] = tmp_740; values[5] = tmp_741; } let tmp_743 = smem_keys[tmp_724 * WPT + 1u]; let tmp_744 = smem_vals[tmp_724 * WPT + 1u]; let tmp_745 = keys[6] < tmp_743 || (keys[6] == tmp_743 && values[6] < tmp_744); if tmp_723 == tmp_745 { keys[6] = tmp_743; values[6] = tmp_744; } let tmp_746 = smem_keys[tmp_724 * WPT + 0u]; let tmp_747 = smem_vals[tmp_724 * WPT + 0u]; let tmp_748 = keys[7] < tmp_746 || (keys[7] == tmp_746 && values[7] < tmp_747); if tmp_723 == tmp_748 { keys[7] = tmp_746; values[7] = tmp_747; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_749 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_750 = seg_base + (local_tid ^ 32u); let tmp_751 = smem_keys[tmp_750 * WPT + 0u]; let tmp_752 = smem_vals[tmp_750 * WPT + 0u]; let tmp_753 = keys[0] < tmp_751 || (keys[0] == tmp_751 && values[0] < tmp_752); if tmp_749 == tmp_753 { keys[0] = tmp_751; values[0] = tmp_752; } let tmp_754 = smem_keys[tmp_750 * WPT + 1u]; let tmp_755 = smem_vals[tmp_750 * WPT + 1u]; let tmp_756 = keys[1] < tmp_754 || (keys[1] == tmp_754 && values[1] < tmp_755); if tmp_749 == tmp_756 { keys[1] = tmp_754; values[1] = tmp_755; } let tmp_757 = smem_keys[tmp_750 * WPT + 2u]; let tmp_758 = smem_vals[tmp_750 * WPT + 2u]; let tmp_759 = keys[2] < tmp_757 || (keys[2] == tmp_757 && values[2] < tmp_758); if tmp_749 == tmp_759 { keys[2] = tmp_757; values[2] = tmp_758; } let tmp_760 = smem_keys[tmp_750 * WPT + 3u]; let tmp_761 = smem_vals[tmp_750 * WPT + 3u]; let tmp_762 = keys[3] < tmp_760 || (keys[3] == tmp_760 && values[3] < tmp_761); if tmp_749 == tmp_762 { keys[3] = tmp_760; values[3] = tmp_761; } let tmp_763 = smem_keys[tmp_750 * WPT + 4u]; let tmp_764 = smem_vals[tmp_750 * WPT + 4u]; let tmp_765 = keys[4] < tmp_763 || (keys[4] == tmp_763 && values[4] < tmp_764); if tmp_749 == tmp_765 { keys[4] = tmp_763; values[4] = tmp_764; } let tmp_766 = smem_keys[tmp_750 * WPT + 5u]; let tmp_767 = smem_vals[tmp_750 * WPT + 5u]; let tmp_768 = keys[5] < tmp_766 || (keys[5] == tmp_766 && values[5] < tmp_767); if tmp_749 == tmp_768 { keys[5] = tmp_766; values[5] = tmp_767; } let tmp_769 = smem_keys[tmp_750 * WPT + 6u]; let tmp_770 = smem_vals[tmp_750 * WPT + 6u]; let tmp_771 = keys[6] < tmp_769 || (keys[6] == tmp_769 && values[6] < tmp_770); if tmp_749 == tmp_771 { keys[6] = tmp_769; values[6] = tmp_770; } let tmp_772 = smem_keys[tmp_750 * WPT + 7u]; let tmp_773 = smem_vals[tmp_750 * WPT + 7u]; let tmp_774 = keys[7] < tmp_772 || (keys[7] == tmp_772 && values[7] < tmp_773); if tmp_749 == tmp_774 { keys[7] = tmp_772; values[7] = tmp_773; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_775 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_776 = seg_base + (local_tid ^ 16u); let tmp_777 = smem_keys[tmp_776 * WPT + 0u]; let tmp_778 = smem_vals[tmp_776 * WPT + 0u]; let tmp_779 = keys[0] < tmp_777 || (keys[0] == tmp_777 && values[0] < tmp_778); if tmp_775 == tmp_779 { keys[0] = tmp_777; values[0] = tmp_778; } let tmp_780 = smem_keys[tmp_776 * WPT + 1u]; let tmp_781 = smem_vals[tmp_776 * WPT + 1u]; let tmp_782 = keys[1] < tmp_780 || (keys[1] == tmp_780 && values[1] < tmp_781); if tmp_775 == tmp_782 { keys[1] = tmp_780; values[1] = tmp_781; } let tmp_783 = smem_keys[tmp_776 * WPT + 2u]; let tmp_784 = smem_vals[tmp_776 * WPT + 2u]; let tmp_785 = keys[2] < tmp_783 || (keys[2] == tmp_783 && values[2] < tmp_784); if tmp_775 == tmp_785 { keys[2] = tmp_783; values[2] = tmp_784; } let tmp_786 = smem_keys[tmp_776 * WPT + 3u]; let tmp_787 = smem_vals[tmp_776 * WPT + 3u]; let tmp_788 = keys[3] < tmp_786 || (keys[3] == tmp_786 && values[3] < tmp_787); if tmp_775 == tmp_788 { keys[3] = tmp_786; values[3] = tmp_787; } let tmp_789 = smem_keys[tmp_776 * WPT + 4u]; let tmp_790 = smem_vals[tmp_776 * WPT + 4u]; let tmp_791 = keys[4] < tmp_789 || (keys[4] == tmp_789 && values[4] < tmp_790); if tmp_775 == tmp_791 { keys[4] = tmp_789; values[4] = tmp_790; } let tmp_792 = smem_keys[tmp_776 * WPT + 5u]; let tmp_793 = smem_vals[tmp_776 * WPT + 5u]; let tmp_794 = keys[5] < tmp_792 || (keys[5] == tmp_792 && values[5] < tmp_793); if tmp_775 == tmp_794 { keys[5] = tmp_792; values[5] = tmp_793; } let tmp_795 = smem_keys[tmp_776 * WPT + 6u]; let tmp_796 = smem_vals[tmp_776 * WPT + 6u]; let tmp_797 = keys[6] < tmp_795 || (keys[6] == tmp_795 && values[6] < tmp_796); if tmp_775 == tmp_797 { keys[6] = tmp_795; values[6] = tmp_796; } let tmp_798 = smem_keys[tmp_776 * WPT + 7u]; let tmp_799 = smem_vals[tmp_776 * WPT + 7u]; let tmp_800 = keys[7] < tmp_798 || (keys[7] == tmp_798 && values[7] < tmp_799); if tmp_775 == tmp_800 { keys[7] = tmp_798; values[7] = tmp_799; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_801 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_802 = seg_base + (local_tid ^ 8u); let tmp_803 = smem_keys[tmp_802 * WPT + 0u]; let tmp_804 = smem_vals[tmp_802 * WPT + 0u]; let tmp_805 = keys[0] < tmp_803 || (keys[0] == tmp_803 && values[0] < tmp_804); if tmp_801 == tmp_805 { keys[0] = tmp_803; values[0] = tmp_804; } let tmp_806 = smem_keys[tmp_802 * WPT + 1u]; let tmp_807 = smem_vals[tmp_802 * WPT + 1u]; let tmp_808 = keys[1] < tmp_806 || (keys[1] == tmp_806 && values[1] < tmp_807); if tmp_801 == tmp_808 { keys[1] = tmp_806; values[1] = tmp_807; } let tmp_809 = smem_keys[tmp_802 * WPT + 2u]; let tmp_810 = smem_vals[tmp_802 * WPT + 2u]; let tmp_811 = keys[2] < tmp_809 || (keys[2] == tmp_809 && values[2] < tmp_810); if tmp_801 == tmp_811 { keys[2] = tmp_809; values[2] = tmp_810; } let tmp_812 = smem_keys[tmp_802 * WPT + 3u]; let tmp_813 = smem_vals[tmp_802 * WPT + 3u]; let tmp_814 = keys[3] < tmp_812 || (keys[3] == tmp_812 && values[3] < tmp_813); if tmp_801 == tmp_814 { keys[3] = tmp_812; values[3] = tmp_813; } let tmp_815 = smem_keys[tmp_802 * WPT + 4u]; let tmp_816 = smem_vals[tmp_802 * WPT + 4u]; let tmp_817 = keys[4] < tmp_815 || (keys[4] == tmp_815 && values[4] < tmp_816); if tmp_801 == tmp_817 { keys[4] = tmp_815; values[4] = tmp_816; } let tmp_818 = smem_keys[tmp_802 * WPT + 5u]; let tmp_819 = smem_vals[tmp_802 * WPT + 5u]; let tmp_820 = keys[5] < tmp_818 || (keys[5] == tmp_818 && values[5] < tmp_819); if tmp_801 == tmp_820 { keys[5] = tmp_818; values[5] = tmp_819; } let tmp_821 = smem_keys[tmp_802 * WPT + 6u]; let tmp_822 = smem_vals[tmp_802 * WPT + 6u]; let tmp_823 = keys[6] < tmp_821 || (keys[6] == tmp_821 && values[6] < tmp_822); if tmp_801 == tmp_823 { keys[6] = tmp_821; values[6] = tmp_822; } let tmp_824 = smem_keys[tmp_802 * WPT + 7u]; let tmp_825 = smem_vals[tmp_802 * WPT + 7u]; let tmp_826 = keys[7] < tmp_824 || (keys[7] == tmp_824 && values[7] < tmp_825); if tmp_801 == tmp_826 { keys[7] = tmp_824; values[7] = tmp_825; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:8) 
    {
    ...
    if keys[0] > keys[4] || (keys[0] == keys[4] && values[0] > values[4]) {
    // swap(0,4) 
    { let tmp_902 = keys[0]; keys[0] = keys[4]; keys[4] = tmp_902;let tmp_903 = values[0]; values[0] = values[4]; values[4] = tmp_903; }
    }
    // cmp_swap(1,5)
    if keys[1] > keys[5] || (keys[1] == keys[5] && values[1] > values[5]) {
    // swap(1,5) 
    { let tmp_904 = keys[1]; keys[1] = keys[5]; keys[5] = tmp_904;let tmp_905 = values[1]; values[1] = values[5]; values[5] = tmp_905; }
    }
    // cmp_swap(2,6)
    if keys[2] > keys[6] || (keys[2] == keys[6] && values[2] > values[6]) {
    // swap(2,6) 
    { let tmp_906 = keys[2]; keys[2] = keys[6]; keys[6] = tmp_906;let tmp_907 = values[2]; values[2] = values[6]; values[6] = tmp_907; }
    }
    // cmp_swap(3,7)
    if keys[3] > keys[7] || (keys[3] == keys[7] && values[3] > values[7]) {
    // swap(3,7) 
    { let tmp_908 = keys[3]; keys[3] = keys[7]; keys[7] = tmp_908;let tmp_909 = values[3]; values[3] = values[7]; values[7] = tmp_909; }
    }
    // exch_local(2,8) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_910 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_910;let tmp_911 = values[0]; values[0] = values[2]; values[2] = tmp_911; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_912 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_912;let tmp_913 = values[1]; values[1] = values[3]; values[3] = tmp_913; }
    }
    // cmp_swap(4,6)
    if keys[4] > keys[6] || (keys[4] == keys[6] && values[4] > values[6]) {
    // swap(4,6) 
    { let tmp_914 = keys[4]; keys[4] = keys[6]; keys[6] = tmp_914;let tmp_915 = values[4]; values[4] = values[6]; values[6] = tmp_915; }
    }
    // cmp_swap(5,7)
    if keys[5] > keys[7] || (keys[5] == keys[7] && values[5] > values[7]) {
    // swap(5,7) 
    { let tmp_916 = keys[5]; keys[5] = keys[7]; keys[7] = tmp_916;let tmp_917 = values[5]; values[5] = values[7]; values[7] = tmp_917; }
    }
    // exch_local(1,8) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_918 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_918;let tmp_919 = values[0]; values[0] = values[1]; values[1] = tmp_919; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_920 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_920;let tmp_921 = values[2]; values[2] = values[3]; values[3] = tmp_921; }
    }
    // cmp_swap(4,5)
    if keys[4] > keys[5] || (keys[4] == keys[5] && values[4] > values[5]) {
    // swap(4,5) 
    { let tmp_922 = keys[4]; keys[4] = keys[5]; keys[5] = tmp_922;let tmp_923 = values[4]; values[4] = values[5]; values[5] = tmp_923; }
    }
    // cmp_swap(6,7)
    if keys[6] > keys[7] || (keys[6] == keys[7] && values[6] > values[7]) {
    // swap(6,7) 
    { let tmp_924 = keys[6]; keys[6] = keys[7]; keys[7] = tmp_924;let tmp_925 = values[6]; values[6] = values[7]; values[7] = tmp_925; }
    }
    // exch_intxn(tmask:255,swbit:7,wpt:8)
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_926 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_927 = seg_base + (local_tid ^ 255u); let tmp_928 = smem_keys[tmp_927 * WPT + 7u]; let tmp_929 = smem_vals[tmp_927 * WPT + 7u]; let tmp_930 = keys[0] < tmp_928 || (keys[0] == tmp_928 && values[0] < tmp_929); if tmp_926 == tmp_930 { keys[0] = tmp_928; values[0] = tmp_929; } let tmp_931 = smem_keys[tmp_927 * WPT + 6u]; let tmp_932 = smem_vals[tmp_927 * WPT + 6u]; let tmp_933 = keys[1] < tmp_931 || (keys[1] == tmp_931 && values[1] < tmp_932); if tmp_926 == tmp_933 { keys[1] = tmp_931; values[1] = tmp_932; } let tmp_934 = smem_keys[tmp_927 * WPT + 5u]; let tmp_935 = smem_vals[tmp_927 * WPT + 5u]; let tmp_936 = keys[2] < tmp_934 || (keys[2] == tmp_934 && values[2] < tmp_935); if tmp_926 == tmp_936 { keys[2] = tmp_934; values[2] = tmp_935; } let tmp_937 = smem_keys[tmp_927 * WPT + 4u]; let tmp_938 = smem_vals[tmp_927 * WPT + 4u]; let tmp_939 = keys[3] < tmp_937 || (keys[3] == tmp_937 && values[3] < tmp_938); if tmp_926 == tmp_939 { keys[3] = tmp_937; values[3] = tmp_938; } let tmp_940 = smem_keys[tmp_927 * WPT + 3u]; let tmp_941 = smem_vals[tmp_927 * WPT + 3u]; let tmp_942 = keys[4] < tmp_940 || (keys[4] == tmp_940 && values[4] < tmp_941); if tmp_926 == tmp_942 { keys[4] = tmp_940; values[4] = tmp_941; } let tmp_943 = smem_keys[tmp_927 * WPT + 2u]; let tmp_944 = smem_vals[tmp_927 * WPT + 2u]; let tmp_945 = keys[5] < tmp_943 || (keys[5] == tmp_943 && values[5] < tmp_944); if tmp_926 == tmp_945 { keys[5] = tmp_943; values[5] = tmp_944; } let tmp_946 = smem_keys[tmp_927 * WPT + 1u]; let tmp_947 = smem_vals[tmp_927 * WPT + 1u]; let tmp_948 = keys[6] < tmp_946 || (keys[6] == tmp_946 && values[6] < tmp_947); if tmp_926 == tmp_948 { keys[6] = tmp_946; values[6] = tmp_947; } let tmp_949 = smem_keys[tmp_927 * WPT + 0u]; let tmp_950 = smem_vals[tmp_927 * WPT + 0u]; let tmp_951 = keys[7] < tmp_949 || (keys[7] == tmp_949 && values[7] < tmp_950); if tmp_926 == tmp_951 { keys[7] = tmp_949; values[7] = tmp_950; } workgroupBarrier(); }
    // exch_paral(tmask:64,swbit:6,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_952 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_953 = seg_base + (local_tid ^ 64u); let tmp_954 = smem_keys[tmp_953 * WPT + 0u]; let tmp_955 = smem_vals[tmp_953 * WPT + 0u]; let tmp_956 = keys[0] < tmp_954 || (keys[0] == tmp_954 && values[0] < tmp_955); if tmp_952 == tmp_956 { keys[0] = tmp_954; values[0] = tmp_955; } let tmp_957 = smem_keys[tmp_953 * WPT + 1u]; let tmp_958 = smem_vals[tmp_953 * WPT + 1u]; let tmp_959 = keys[1] < tmp_957 || (keys[1] == tmp_957 && values[1] < tmp_958); if tmp_952 == tmp_959 { keys[1] = tmp_957; values[1] = tmp_958; } let tmp_960 = smem_keys[tmp_953 * WPT + 2u]; let tmp_961 = smem_vals[tmp_953 * WPT + 2u]; let tmp_962 = keys[2] < tmp_960 || (keys[2] == tmp_960 && values[2] < tmp_961); if tmp_952 == tmp_962 { keys[2] = tmp_960; values[2] = tmp_961; } let tmp_963 = smem_keys[tmp_953 * WPT + 3u]; let tmp_964 = smem_vals[tmp_953 * WPT + 3u]; let tmp_965 = keys[3] < tmp_963 || (keys[3] == tmp_963 && values[3] < tmp_964); if tmp_952 == tmp_965 { keys[3] = tmp_963; values[3] = tmp_964; } let tmp_966 = smem_keys[tmp_953 * WPT + 4u]; let tmp_967 = smem_vals[tmp_953 * WPT + 4u]; let tmp_968 = keys[4] < tmp_966 || (keys[4] == tmp_966 && values[4] < tmp_967); if tmp_952 == tmp_968 { keys[4] = tmp_966; values[4] = tmp_967; } let tmp_969 = smem_keys[tmp_953 * WPT + 5u]; let tmp_970 = smem_vals[tmp_953 * WPT + 5u]; let tmp_971 = keys[5] < tmp_969 || (keys[5] == tmp_969 && values[5] < tmp_970); if tmp_952 == tmp_971 { keys[5] = tmp_969; values[5] = tmp_970; } let tmp_972 = smem_keys[tmp_953 * WPT + 6u]; let tmp_973 = smem_vals[tmp_953 * WPT + 6u]; let tmp_974 = keys[6] < tmp_972 || (keys[6] == tmp_972 && values[6] < tmp_973); if tmp_952 == tmp_974 { keys[6] = tmp_972; values[6] = tmp_973; } let tmp_975 = smem_keys[tmp_953 * WPT + 7u]; let tmp_976 = smem_vals[tmp_953 * WPT + 7u]; let tmp_977 = keys[7] < tmp_975 || (keys[7] == tmp_975 && values[7] < tmp_976); if tmp_952 == tmp_977 { keys[7] = tmp_975; values[7] = tmp_976; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_978 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_979 = seg_base + (local_tid ^ 32u); let tmp_980 = smem_keys[tmp_979 * WPT + 0u]; let tmp_981 = smem_vals[tmp_979 * WPT + 0u]; let tmp_982 = keys[0] < tmp_980 || (keys[0] == tmp_980 && values[0] < tmp_981); if tmp_978 == tmp_982 { keys[0] = tmp_980; values[0] = tmp_981; } let tmp_983 = smem_keys[tmp_979 * WPT + 1u]; let tmp_984 = smem_vals[tmp_979 * WPT + 1u]; let tmp_985 = keys[1] < tmp_983 || (keys[1] == tmp_983 && values[1] < tmp_984); if tmp_978 == tmp_985 { keys[1] = tmp_983; values[1] = tmp_984; } let tmp_986 = smem_keys[tmp_979 * WPT + 2u]; let tmp_987 = smem_vals[tmp_979 * WPT + 2u]; let tmp_988 = keys[2] < tmp_986 || (keys[2] == tmp_986 && values[2] < tmp_987); if tmp_978 == tmp_988 { keys[2] = tmp_986; values[2] = tmp_987; } let tmp_989 = smem_keys[tmp_979 * WPT + 3u]; let tmp_990 = smem_vals[tmp_979 * WPT + 3u]; let tmp_991 = keys[3] < tmp_989 || (keys[3] == tmp_989 && values[3] < tmp_990); if tmp_978 == tmp_991 { keys[3] = tmp_989; values[3] = tmp_990; } let tmp_992 = smem_keys[tmp_979 * WPT + 4u]; let tmp_993 = smem_vals[tmp_979 * WPT + 4u]; let tmp_994 = keys[4] < tmp_992 || (keys[4] == tmp_992 && values[4] < tmp_993); if tmp_978 == tmp_994 { keys[4] = tmp_992; values[4] = tmp_993; } let tmp_995 = smem_keys[tmp_979 * WPT + 5u]; let tmp_996 = smem_vals[tmp_979 * WPT + 5u]; let tmp_997 = keys[5] < tmp_995 || (keys[5] == tmp_995 && values[5] < tmp_996); if tmp_978 == tmp_997 { keys[5] = tmp_995; values[5] = tmp_996; } let tmp_998 = smem_keys[tmp_979 * WPT + 6u]; let tmp_999 = smem_vals[tmp_979 * WPT + 6u]; let tmp_1000 = keys[6] < tmp_998 || (keys[6] == tmp_998 && values[6] < tmp_999); if tmp_978 == tmp_1000 { keys[6] = tmp_998; values[6] = tmp_999; } let tmp_1001 = smem_keys[tmp_979 * WPT + 7u]; let tmp_1002 = smem_vals[tmp_979 * WPT + 7u]; let tmp_1003 = keys[7] < tmp_1001 || (keys[7] == tmp_1001 && values[7] < tmp_1002); if tmp_978 == tmp_1003 { keys[7] = tmp_1001; values[7] = tmp_1002; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_1004 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_1005 = seg_base + (local_tid ^ 16u); let tmp_1006 = smem_keys[tmp_1005 * WPT + 0u]; let tmp_1007 = smem_vals[tmp_1005 * WPT + 0u]; let tmp_1008 = keys[0] < tmp_1006 || (keys[0] == tmp_1006 && values[0] < tmp_1007); if tmp_1004 == tmp_1008 { keys[0] = tmp_1006; values[0] = tmp_1007; } let tmp_1009 = smem_keys[tmp_1005 * WPT + 1u]; let tmp_1010 = smem_vals[tmp_1005 * WPT + 1u]; let tmp_1011 = keys[1] < tmp_1009 || (keys[1] == tmp_1009 && values[1] < tmp_1010); if tmp_1004 == tmp_1011 { keys[1] = tmp_1009; values[1] = tmp_1010; } let tmp_1012 = smem_keys[tmp_1005 * WPT + 2u]; let tmp_1013 = smem_vals[tmp_1005 * WPT + 2u]; let tmp_1014 = keys[2] < tmp_1012 || (keys[2] == tmp_1012 && values[2] < tmp_1013); if tmp_1004 == tmp_1014 { keys[2] = tmp_1012; values[2] = tmp_1013; } let tmp_1015 = smem_keys[tmp_1005 * WPT + 3u]; let tmp_1016 = smem_vals[tmp_1005 * WPT + 3u]; let tmp_1017 = keys[3] < tmp_1015 || (keys[3] == tmp_1015 && values[3] < tmp_1016); if tmp_1004 == tmp_1017 { keys[3] = tmp_1015; values[3] = tmp_1016; } let tmp_1018 = smem_keys[tmp_1005 * WPT + 4u]; let tmp_1019 = smem_vals[tmp_1005 * WPT + 4u]; let tmp_1020 = keys[4] < tmp_1018 || (keys[4] == tmp_1018 && values[4] < tmp_1019); if tmp_1004 == tmp_1020 { keys[4] = tmp_1018; values[4] = tmp_1019; } let tmp_1021 = smem_keys[tmp_1005 * WPT + 5u]; let tmp_1022 = smem_vals[tmp_1005 * WPT + 5u]; let tmp_1023 = keys[5] < tmp_1021 || (keys[5] == tmp_1021 && values[5] < tmp_1022); if tmp_1004 == tmp_1023 { keys[5] = tmp_1021; values[5] = tmp_1022; } let tmp_1024 = smem_keys[tmp_1005 * WPT + 6u]; let tmp_1025 = smem_vals[tmp_1005 * WPT + 6u]; let tmp_1026 = keys[6] < tmp_1024 || (keys[6] == tmp_1024 && values[6] < tmp_1025); if tmp_1004 == tmp_1026 { keys[6] = tmp_1024; values[6] = tmp_1025; } let tmp_1027 = smem_keys[tmp_1005 * WPT + 7u]; let tmp_1028 = smem_vals[tmp_1005 * WPT + 7u]; let tmp_1029 = keys[7] < tmp_1027 || (keys[7] == tmp_1027 && values[7] < tmp_1028); if tmp_1004 == tmp_1029 { keys[7] = tmp_1027; values[7] = tmp_1028; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:8) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_1030 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_1031 = seg_base + (local_tid ^ 8u); let tmp_1032 = smem_keys[tmp_1031 * WPT + 0u]; let tmp_1033 = smem_vals[tmp_1031 * WPT + 0u]; let tmp_1034 = keys[0] < tmp_1032 || (keys[0] == tmp_1032 && values[0] < tmp_1033); if tmp_1030 == tmp_1034 { keys[0] = tmp_1032; values[0] = tmp_1033; } let tmp_1035 = smem_keys[tmp_1031 * WPT + 1u]; let tmp_1036 = smem_vals[tmp_1031 * WPT + 1u]; let tmp_1037 = keys[1] < tmp_1035 || (keys[1] == tmp_1035 && values[1] < tmp_1036); if tmp_1030 == tmp_1037 { keys[1] = tmp_1035; values[1] = tmp_1036; } let tmp_1038 = smem_keys[tmp_1031 * WPT + 2u]; let tmp_1039 = smem_vals[tmp_1031 * WPT + 2u]; let tmp_1040 = keys[2] < tmp_1038 || (keys[2] == tmp_1038 && values[2] < tmp_1039); if tmp_1030 == tmp_1040 { keys[2] = tmp_1038; values[2] = tmp_1039; } let tmp_1041 = smem_keys[tmp_1031 * WPT + 3u]; let tmp_1042 = smem_vals[tmp_1031 * WPT + 3u]; let tmp_1043 = keys[3] < tmp_1041 || (keys[3] == tmp_1041 && values[3] < tmp_1042); if tmp_1030 == tmp_1043 { keys[3] = tmp_1041; values[3] = tmp_1042; } let tmp_1044 = smem_keys[tmp_1031 * WPT + 4u]; let tmp_1045 = smem_vals[tmp_1031 * WPT + 4u]; let tmp_1046 = keys[4] < tmp_1044 || (keys[4] == tmp_1044 && values[4] < tmp_1045); if tmp_1030 == tmp_1046 { keys[4] = tmp_1044; values[4] = tmp_1045; } let tmp_1047 = smem_keys[tmp_1031 * WPT + 5u]; let tmp_1048 = smem_vals[tmp_1031 * WPT + 5u]; let tmp_1049 = keys[5] < tmp_1047 || (keys[5] == tmp_1047 && values[5] < tmp_1048); if tmp_1030 == tmp_1049 { keys[5] = tmp_1047; values[5] = tmp_1048; } let tmp_1050 = smem_keys[tmp_1031 * WPT + 6u]; let tmp_1051 = smem_vals[tmp_1031 * WPT + 6u]; let tmp_1052 = keys[6] < tmp_1050 || (keys[6] == tmp_1050 && values[6] < tmp_1051); if tmp_1030 == tmp_1052 { keys[6] = tmp_1050; values[6] = tmp_1051; } let tmp_1053 = smem_keys[tmp_1031 * WPT + 7u]; let tmp_1054 = smem_vals[tmp_1031 * WPT + 7u]; let tmp_1055 = keys[7] < tmp_1053 || (keys[7] == tmp_1053 && values[7] < tmp_1054); if tmp_1030 == tmp_1055 { keys[7] = tmp_1053; values[7] = tmp_1054; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:8) 
    {
    ...
    if keys[0] > keys[4] || (keys[0] == keys[4] && values[0] > values[4]) {
    // swap(0,4) 
    { let tmp_1131 = keys[0]; keys[0] = keys[4]; keys[4] = tmp_1131;let tmp_1132 = values[0]; values[0] = values[4]; values[4] = tmp_1132; }
    }
    // cmp_swap(1,5)
    if keys[1] > keys[5] || (keys[1] == keys[5] && values[1] > values[5]) {
    // swap(1,5) 
    { let tmp_1133 = keys[1]; keys[1] = keys[5]; keys[5] = tmp_1133;let tmp_1134 = values[1]; values[1] = values[5]; values[5] = tmp_1134; }
    }
    // cmp_swap(2,6)
    if keys[2] > keys[6] || (keys[2] == keys[6] && values[2] > values[6]) {
    // swap(2,6) 
    { let tmp_1135 = keys[2]; keys[2] = keys[6]; keys[6] = tmp_1135;let tmp_1136 = values[2]; values[2] = values[6]; values[6] = tmp_1136; }
    }
    // cmp_swap(3,7)
    if keys[3] > keys[7] || (keys[3] == keys[7] && values[3] > values[7]) {
    // swap(3,7) 
    { let tmp_1137 = keys[3]; keys[3] = keys[7]; keys[7] = tmp_1137;let tmp_1138 = values[3]; values[3] = values[7]; values[7] = tmp_1138; }
    }
    // exch_local(2,8) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_1139 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_1139;let tmp_1140 = values[0]; values[0] = values[2]; values[2] = tmp_1140; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_1141 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_1141;let tmp_1142 = values[1]; values[1] = values[3]; values[3] = tmp_1142; }
    }
    // cmp_swap(4,6)
    if keys[4] > keys[6] || (keys[4] == keys[6] && values[4] > values[6]) {
    // swap(4,6) 
    { let tmp_1143 = keys[4]; keys[4] = keys[6]; keys[6] = tmp_1143;let tmp_1144 = values[4]; values[4] = values[6]; values[6] = tmp_1144; }
    }
    // cmp_swap(5,7)
    if keys[5] > keys[7] || (keys[5] == keys[7] && values[5] > values[7]) {
    // swap(5,7) 
    { let tmp_1145 = keys[5]; keys[5] = keys[7]; keys[7] = tmp_1145;let tmp_1146 = values[5]; values[5] = values[7]; values[7] = tmp_1146; }
    }
    // exch_local(1,8) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_1147 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_1147;let tmp_1148 = values[0]; values[0] = values[1]; values[1] = tmp_1148; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_1149 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_1149;let tmp_1150 = values[2]; values[2] = values[3]; values[3] = tmp_1150; }
    }
    // cmp_swap(4,5)
    if keys[4] > keys[5] || (keys[4] == keys[5] && values[4] > values[5]) {
    // swap(4,5) 
    { let tmp_1151 = keys[4]; keys[4] = keys[5]; keys[5] = tmp_1151;let tmp_1152 = values[4]; values[4] = values[5]; values[5] = tmp_1152; }
    }
    // cmp_swap(6,7)
    if keys[6] > keys[7] || (keys[6] == keys[7] && values[6] > values[7]) {
    // swap(6,7) 
    { let tmp_1153 = keys[6]; keys[6] = keys[7]; keys[7] = tmp_1153;let tmp_1154 = values[6]; values[6] = values[7]; values[7] = tmp_1154; }
    }
