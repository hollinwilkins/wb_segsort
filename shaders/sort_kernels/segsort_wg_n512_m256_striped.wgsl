...
fn segsort_wg_n512_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_240 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_241 = seg_base + (local_tid ^ 255u); let tmp_242 = smem_keys[tmp_241 * WPT + 1u]; let tmp_243 = smem_vals[tmp_241 * WPT + 1u]; let tmp_244 = keys[0] < tmp_242 || (keys[0] == tmp_242 && values[0] < tmp_243); if tmp_240 == tmp_244 { keys[0] = tmp_242; values[0] = tmp_243; } let tmp_245 = smem_keys[tmp_241 * WPT + 0u]; let tmp_246 = smem_vals[tmp_241 * WPT + 0u]; let tmp_247 = keys[1] < tmp_245 || (keys[1] == tmp_245 && values[1] < tmp_246); if tmp_240 == tmp_247 { keys[1] = tmp_245; values[1] = tmp_246; } workgroupBarrier(); }
    // exch_paral(tmask:64,swbit:6,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_248 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_249 = seg_base + (local_tid ^ 64u); let tmp_250 = smem_keys[tmp_249 * WPT + 0u]; let tmp_251 = smem_vals[tmp_249 * WPT + 0u]; let tmp_252 = keys[0] < tmp_250 || (keys[0] == tmp_250 && values[0] < tmp_251); if tmp_248 == tmp_252 { keys[0] = tmp_250; values[0] = tmp_251; } let tmp_253 = smem_keys[tmp_249 * WPT + 1u]; let tmp_254 = smem_vals[tmp_249 * WPT + 1u]; let tmp_255 = keys[1] < tmp_253 || (keys[1] == tmp_253 && values[1] < tmp_254); if tmp_248 == tmp_255 { keys[1] = tmp_253; values[1] = tmp_254; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_256 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_257 = seg_base + (local_tid ^ 32u); let tmp_258 = smem_keys[tmp_257 * WPT + 0u]; let tmp_259 = smem_vals[tmp_257 * WPT + 0u]; let tmp_260 = keys[0] < tmp_258 || (keys[0] == tmp_258 && values[0] < tmp_259); if tmp_256 == tmp_260 { keys[0] = tmp_258; values[0] = tmp_259; } let tmp_261 = smem_keys[tmp_257 * WPT + 1u]; let tmp_262 = smem_vals[tmp_257 * WPT + 1u]; let tmp_263 = keys[1] < tmp_261 || (keys[1] == tmp_261 && values[1] < tmp_262); if tmp_256 == tmp_263 { keys[1] = tmp_261; values[1] = tmp_262; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_264 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_265 = seg_base + (local_tid ^ 16u); let tmp_266 = smem_keys[tmp_265 * WPT + 0u]; let tmp_267 = smem_vals[tmp_265 * WPT + 0u]; let tmp_268 = keys[0] < tmp_266 || (keys[0] == tmp_266 && values[0] < tmp_267); if tmp_264 == tmp_268 { keys[0] = tmp_266; values[0] = tmp_267; } let tmp_269 = smem_keys[tmp_265 * WPT + 1u]; let tmp_270 = smem_vals[tmp_265 * WPT + 1u]; let tmp_271 = keys[1] < tmp_269 || (keys[1] == tmp_269 && values[1] < tmp_270); if tmp_264 == tmp_271 { keys[1] = tmp_269; values[1] = tmp_270; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_272 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_273 = seg_base + (local_tid ^ 8u); let tmp_274 = smem_keys[tmp_273 * WPT + 0u]; let tmp_275 = smem_vals[tmp_273 * WPT + 0u]; let tmp_276 = keys[0] < tmp_274 || (keys[0] == tmp_274 && values[0] < tmp_275); if tmp_272 == tmp_276 { keys[0] = tmp_274; values[0] = tmp_275; } let tmp_277 = smem_keys[tmp_273 * WPT + 1u]; let tmp_278 = smem_vals[tmp_273 * WPT + 1u]; let tmp_279 = keys[1] < tmp_277 || (keys[1] == tmp_277 && values[1] < tmp_278); if tmp_272 == tmp_279 { keys[1] = tmp_277; values[1] = tmp_278; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_280 = extractBits(local_tid, 2u, 1u) != 0u; let tmp_281 = seg_base + (local_tid ^ 4u); let tmp_282 = smem_keys[tmp_281 * WPT + 0u]; let tmp_283 = smem_vals[tmp_281 * WPT + 0u]; let tmp_284 = keys[0] < tmp_282 || (keys[0] == tmp_282 && values[0] < tmp_283); if tmp_280 == tmp_284 { keys[0] = tmp_282; values[0] = tmp_283; } let tmp_285 = smem_keys[tmp_281 * WPT + 1u]; let tmp_286 = smem_vals[tmp_281 * WPT + 1u]; let tmp_287 = keys[1] < tmp_285 || (keys[1] == tmp_285 && values[1] < tmp_286); if tmp_280 == tmp_287 { keys[1] = tmp_285; values[1] = tmp_286; } workgroupBarrier(); }
    // exch_paral(tmask:2,swbit:1,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_288 = extractBits(local_tid, 1u, 1u) != 0u; let tmp_289 = seg_base + (local_tid ^ 2u); let tmp_290 = smem_keys[tmp_289 * WPT + 0u]; let tmp_291 = smem_vals[tmp_289 * WPT + 0u]; let tmp_292 = keys[0] < tmp_290 || (keys[0] == tmp_290 && values[0] < tmp_291); if tmp_288 == tmp_292 { keys[0] = tmp_290; values[0] = tmp_291; } let tmp_293 = smem_keys[tmp_289 * WPT + 1u]; let tmp_294 = smem_vals[tmp_289 * WPT + 1u]; let tmp_295 = keys[1] < tmp_293 || (keys[1] == tmp_293 && values[1] < tmp_294); if tmp_288 == tmp_295 { keys[1] = tmp_293; values[1] = tmp_294; } workgroupBarrier(); }
    // exch_paral(tmask:1,swbit:0,wpt:2) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_296 = extractBits(local_tid, 0u, 1u) != 0u; let tmp_297 = seg_base + (local_tid ^ 1u); let tmp_298 = smem_keys[tmp_297 * WPT + 0u]; let tmp_299 = smem_vals[tmp_297 * WPT + 0u]; let tmp_300 = keys[0] < tmp_298 || (keys[0] == tmp_298 && values[0] < tmp_299); if tmp_296 == tmp_300 { keys[0] = tmp_298; values[0] = tmp_299; } let tmp_301 = smem_keys[tmp_297 * WPT + 1u]; let tmp_302 = smem_vals[tmp_297 * WPT + 1u]; let tmp_303 = keys[1] < tmp_301 || (keys[1] == tmp_301 && values[1] < tmp_302); if tmp_296 == tmp_303 { keys[1] = tmp_301; values[1] = tmp_302; } workgroupBarrier(); }
    // exch_local(1,2) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_304 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_304;let tmp_305 = values[0]; values[0] = values[1]; values[1] = tmp_305; }
    }
