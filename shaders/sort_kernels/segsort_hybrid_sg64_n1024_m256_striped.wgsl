...
fn segsort_hybrid_sg64_n1024_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_433 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_434 = seg_base + (local_tid ^ 255u); let tmp_435 = smem_keys[tmp_434 * WPT + 3u]; let tmp_436 = smem_vals[tmp_434 * WPT + 3u]; let tmp_437 = keys[0] < tmp_435 || (keys[0] == tmp_435 && values[0] < tmp_436); if tmp_433 == tmp_437 { keys[0] = tmp_435; values[0] = tmp_436; } let tmp_438 = smem_keys[tmp_434 * WPT + 2u]; let tmp_439 = smem_vals[tmp_434 * WPT + 2u]; let tmp_440 = keys[1] < tmp_438 || (keys[1] == tmp_438 && values[1] < tmp_439); if tmp_433 == tmp_440 { keys[1] = tmp_438; values[1] = tmp_439; } let tmp_441 = smem_keys[tmp_434 * WPT + 1u]; let tmp_442 = smem_vals[tmp_434 * WPT + 1u]; let tmp_443 = keys[2] < tmp_441 || (keys[2] == tmp_441 && values[2] < tmp_442); if tmp_433 == tmp_443 { keys[2] = tmp_441; values[2] = tmp_442; } let tmp_444 = smem_keys[tmp_434 * WPT + 0u]; let tmp_445 = smem_vals[tmp_434 * WPT + 0u]; let tmp_446 = keys[3] < tmp_444 || (keys[3] == tmp_444 && values[3] < tmp_445); if tmp_433 == tmp_446 { keys[3] = tmp_444; values[3] = tmp_445; } workgroupBarrier(); }
    // exch_paral(tmask:64,swbit:6,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_447 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_448 = seg_base + (local_tid ^ 64u); let tmp_449 = smem_keys[tmp_448 * WPT + 0u]; let tmp_450 = smem_vals[tmp_448 * WPT + 0u]; let tmp_451 = keys[0] < tmp_449 || (keys[0] == tmp_449 && values[0] < tmp_450); if tmp_447 == tmp_451 { keys[0] = tmp_449; values[0] = tmp_450; } let tmp_452 = smem_keys[tmp_448 * WPT + 1u]; let tmp_453 = smem_vals[tmp_448 * WPT + 1u]; let tmp_454 = keys[1] < tmp_452 || (keys[1] == tmp_452 && values[1] < tmp_453); if tmp_447 == tmp_454 { keys[1] = tmp_452; values[1] = tmp_453; } let tmp_455 = smem_keys[tmp_448 * WPT + 2u]; let tmp_456 = smem_vals[tmp_448 * WPT + 2u]; let tmp_457 = keys[2] < tmp_455 || (keys[2] == tmp_455 && values[2] < tmp_456); if tmp_447 == tmp_457 { keys[2] = tmp_455; values[2] = tmp_456; } let tmp_458 = smem_keys[tmp_448 * WPT + 3u]; let tmp_459 = smem_vals[tmp_448 * WPT + 3u]; let tmp_460 = keys[3] < tmp_458 || (keys[3] == tmp_458 && values[3] < tmp_459); if tmp_447 == tmp_460 { keys[3] = tmp_458; values[3] = tmp_459; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:4) 
    {
    ...
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_539 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_539;let tmp_540 = values[0]; values[0] = values[2]; values[2] = tmp_540; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_541 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_541;let tmp_542 = values[1]; values[1] = values[3]; values[3] = tmp_542; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_543 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_543;let tmp_544 = values[0]; values[0] = values[1]; values[1] = tmp_544; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_545 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_545;let tmp_546 = values[2]; values[2] = values[3]; values[3] = tmp_546; }
    }
