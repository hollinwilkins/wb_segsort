...
fn segsort_hybrid_sg128_n512_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; workgroupBarrier(); let tmp_212 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_213 = seg_base + (local_tid ^ 255u); let tmp_214 = smem_keys[tmp_213 * WPT + 1u]; let tmp_215 = smem_vals[tmp_213 * WPT + 1u]; let tmp_216 = keys[0] < tmp_214 || (keys[0] == tmp_214 && values[0] < tmp_215); if tmp_212 == tmp_216 { keys[0] = tmp_214; values[0] = tmp_215; } let tmp_217 = smem_keys[tmp_213 * WPT + 0u]; let tmp_218 = smem_vals[tmp_213 * WPT + 0u]; let tmp_219 = keys[1] < tmp_217 || (keys[1] == tmp_217 && values[1] < tmp_218); if tmp_212 == tmp_219 { keys[1] = tmp_217; values[1] = tmp_218; } workgroupBarrier(); }
    ...
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_269 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_269;let tmp_270 = values[0]; values[0] = values[1]; values[1] = tmp_270; }
    }
