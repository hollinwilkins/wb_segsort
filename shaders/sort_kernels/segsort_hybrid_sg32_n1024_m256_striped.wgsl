...
fn segsort_hybrid_sg32_n1024_m256_striped(
    ...
    // exch_intxn(tmask:127,swbit:6,wpt:4)
    {
        // _exch_workgroup([(0, 3), (1, 2), (2, 1), (3, 0)],127,6)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_334 = extractBits(local_tid, 6u, 1u) != 0u;
            let tmp_335 = seg_base + (local_tid ^ 127u);
            let tmp_336 = smem_keys[tmp_335 * WPT + 3u];
            let tmp_337 = smem_vals[tmp_335 * WPT + 3u];
            let tmp_338 = keys[0] < tmp_336 || (keys[0] == tmp_336 && values[0] < tmp_337);
            if tmp_334 == tmp_338 { keys[0] = tmp_336; values[0] = tmp_337; }
            let tmp_339 = smem_keys[tmp_335 * WPT + 2u];
            let tmp_340 = smem_vals[tmp_335 * WPT + 2u];
            let tmp_341 = keys[1] < tmp_339 || (keys[1] == tmp_339 && values[1] < tmp_340);
            if tmp_334 == tmp_341 { keys[1] = tmp_339; values[1] = tmp_340; }
            let tmp_342 = smem_keys[tmp_335 * WPT + 1u];
            let tmp_343 = smem_vals[tmp_335 * WPT + 1u];
            let tmp_344 = keys[2] < tmp_342 || (keys[2] == tmp_342 && values[2] < tmp_343);
            if tmp_334 == tmp_344 { keys[2] = tmp_342; values[2] = tmp_343; }
            let tmp_345 = smem_keys[tmp_335 * WPT + 0u];
            let tmp_346 = smem_vals[tmp_335 * WPT + 0u];
            let tmp_347 = keys[3] < tmp_345 || (keys[3] == tmp_345 && values[3] < tmp_346);
            if tmp_334 == tmp_347 { keys[3] = tmp_345; values[3] = tmp_346; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:32,swbit:5,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],32,5)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_348 = extractBits(local_tid, 5u, 1u) != 0u;
            let tmp_349 = seg_base + (local_tid ^ 32u);
            let tmp_350 = smem_keys[tmp_349 * WPT + 0u];
            let tmp_351 = smem_vals[tmp_349 * WPT + 0u];
            let tmp_352 = keys[0] < tmp_350 || (keys[0] == tmp_350 && values[0] < tmp_351);
            if tmp_348 == tmp_352 { keys[0] = tmp_350; values[0] = tmp_351; }
            let tmp_353 = smem_keys[tmp_349 * WPT + 1u];
            let tmp_354 = smem_vals[tmp_349 * WPT + 1u];
            let tmp_355 = keys[1] < tmp_353 || (keys[1] == tmp_353 && values[1] < tmp_354);
            if tmp_348 == tmp_355 { keys[1] = tmp_353; values[1] = tmp_354; }
            let tmp_356 = smem_keys[tmp_349 * WPT + 2u];
            let tmp_357 = smem_vals[tmp_349 * WPT + 2u];
            let tmp_358 = keys[2] < tmp_356 || (keys[2] == tmp_356 && values[2] < tmp_357);
            if tmp_348 == tmp_358 { keys[2] = tmp_356; values[2] = tmp_357; }
            let tmp_359 = smem_keys[tmp_349 * WPT + 3u];
            let tmp_360 = smem_vals[tmp_349 * WPT + 3u];
            let tmp_361 = keys[3] < tmp_359 || (keys[3] == tmp_359 && values[3] < tmp_360);
            if tmp_348 == tmp_361 { keys[3] = tmp_359; values[3] = tmp_360; }
            workgroupBarrier();
        }
    }
    ...
    // exch_local(2,4)
    {
        ...
        if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
            // swap(0,2)
            {
                let tmp_427 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_427;
                let tmp_428 = values[0]; values[0] = values[2]; values[2] = tmp_428;
            }
        }
        // cmp_swap(1,3)
        if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
            // swap(1,3)
            {
                let tmp_429 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_429;
                let tmp_430 = values[1]; values[1] = values[3]; values[3] = tmp_430;
            }
        }
    }
    ...
    // exch_local(1,4)
    {
        ...
        if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
            // swap(0,1)
            {
                let tmp_431 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_431;
                let tmp_432 = values[0]; values[0] = values[1]; values[1] = tmp_432;
            }
        }
        // cmp_swap(2,3)
        if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
            // swap(2,3)
            {
                let tmp_433 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_433;
                let tmp_434 = values[2]; values[2] = values[3]; values[3] = tmp_434;
            }
        }
    }
    ...
    // exch_intxn(tmask:255,swbit:7,wpt:4)
    {
        // _exch_workgroup([(0, 3), (1, 2), (2, 1), (3, 0)],255,7)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_435 = extractBits(local_tid, 7u, 1u) != 0u;
            let tmp_436 = seg_base + (local_tid ^ 255u);
            let tmp_437 = smem_keys[tmp_436 * WPT + 3u];
            let tmp_438 = smem_vals[tmp_436 * WPT + 3u];
            let tmp_439 = keys[0] < tmp_437 || (keys[0] == tmp_437 && values[0] < tmp_438);
            if tmp_435 == tmp_439 { keys[0] = tmp_437; values[0] = tmp_438; }
            let tmp_440 = smem_keys[tmp_436 * WPT + 2u];
            let tmp_441 = smem_vals[tmp_436 * WPT + 2u];
            let tmp_442 = keys[1] < tmp_440 || (keys[1] == tmp_440 && values[1] < tmp_441);
            if tmp_435 == tmp_442 { keys[1] = tmp_440; values[1] = tmp_441; }
            let tmp_443 = smem_keys[tmp_436 * WPT + 1u];
            let tmp_444 = smem_vals[tmp_436 * WPT + 1u];
            let tmp_445 = keys[2] < tmp_443 || (keys[2] == tmp_443 && values[2] < tmp_444);
            if tmp_435 == tmp_445 { keys[2] = tmp_443; values[2] = tmp_444; }
            let tmp_446 = smem_keys[tmp_436 * WPT + 0u];
            let tmp_447 = smem_vals[tmp_436 * WPT + 0u];
            let tmp_448 = keys[3] < tmp_446 || (keys[3] == tmp_446 && values[3] < tmp_447);
            if tmp_435 == tmp_448 { keys[3] = tmp_446; values[3] = tmp_447; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:64,swbit:6,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],64,6)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_449 = extractBits(local_tid, 6u, 1u) != 0u;
            let tmp_450 = seg_base + (local_tid ^ 64u);
            let tmp_451 = smem_keys[tmp_450 * WPT + 0u];
            let tmp_452 = smem_vals[tmp_450 * WPT + 0u];
            let tmp_453 = keys[0] < tmp_451 || (keys[0] == tmp_451 && values[0] < tmp_452);
            if tmp_449 == tmp_453 { keys[0] = tmp_451; values[0] = tmp_452; }
            let tmp_454 = smem_keys[tmp_450 * WPT + 1u];
            let tmp_455 = smem_vals[tmp_450 * WPT + 1u];
            let tmp_456 = keys[1] < tmp_454 || (keys[1] == tmp_454 && values[1] < tmp_455);
            if tmp_449 == tmp_456 { keys[1] = tmp_454; values[1] = tmp_455; }
            let tmp_457 = smem_keys[tmp_450 * WPT + 2u];
            let tmp_458 = smem_vals[tmp_450 * WPT + 2u];
            let tmp_459 = keys[2] < tmp_457 || (keys[2] == tmp_457 && values[2] < tmp_458);
            if tmp_449 == tmp_459 { keys[2] = tmp_457; values[2] = tmp_458; }
            let tmp_460 = smem_keys[tmp_450 * WPT + 3u];
            let tmp_461 = smem_vals[tmp_450 * WPT + 3u];
            let tmp_462 = keys[3] < tmp_460 || (keys[3] == tmp_460 && values[3] < tmp_461);
            if tmp_449 == tmp_462 { keys[3] = tmp_460; values[3] = tmp_461; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:32,swbit:5,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],32,5)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_463 = extractBits(local_tid, 5u, 1u) != 0u;
            let tmp_464 = seg_base + (local_tid ^ 32u);
            let tmp_465 = smem_keys[tmp_464 * WPT + 0u];
            let tmp_466 = smem_vals[tmp_464 * WPT + 0u];
            let tmp_467 = keys[0] < tmp_465 || (keys[0] == tmp_465 && values[0] < tmp_466);
            if tmp_463 == tmp_467 { keys[0] = tmp_465; values[0] = tmp_466; }
            let tmp_468 = smem_keys[tmp_464 * WPT + 1u];
            let tmp_469 = smem_vals[tmp_464 * WPT + 1u];
            let tmp_470 = keys[1] < tmp_468 || (keys[1] == tmp_468 && values[1] < tmp_469);
            if tmp_463 == tmp_470 { keys[1] = tmp_468; values[1] = tmp_469; }
            let tmp_471 = smem_keys[tmp_464 * WPT + 2u];
            let tmp_472 = smem_vals[tmp_464 * WPT + 2u];
            let tmp_473 = keys[2] < tmp_471 || (keys[2] == tmp_471 && values[2] < tmp_472);
            if tmp_463 == tmp_473 { keys[2] = tmp_471; values[2] = tmp_472; }
            let tmp_474 = smem_keys[tmp_464 * WPT + 3u];
            let tmp_475 = smem_vals[tmp_464 * WPT + 3u];
            let tmp_476 = keys[3] < tmp_474 || (keys[3] == tmp_474 && values[3] < tmp_475);
            if tmp_463 == tmp_476 { keys[3] = tmp_474; values[3] = tmp_475; }
            workgroupBarrier();
        }
    }
    ...
    // exch_local(1,4)
    {
        ...
        if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
            // swap(0,1)
            {
                let tmp_546 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_546;
                let tmp_547 = values[0]; values[0] = values[1]; values[1] = tmp_547;
            }
        }
        // cmp_swap(2,3)
        if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
            // swap(2,3)
            {
                let tmp_548 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_548;
                let tmp_549 = values[2]; values[2] = values[3]; values[3] = tmp_549;
            }
        }
    }
