...
fn segsort_hybrid_sg128_n1024_m256_striped(
    ...
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_424 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_424;let tmp_425 = values[0]; values[0] = values[2]; values[2] = tmp_425; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_426 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_426;let tmp_427 = values[1]; values[1] = values[3]; values[3] = tmp_427; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_428 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_428;let tmp_429 = values[0]; values[0] = values[1]; values[1] = tmp_429; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_430 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_430;let tmp_431 = values[2]; values[2] = values[3]; values[3] = tmp_431; }
    }
    // exch_intxn(tmask:255,swbit:7,wpt:4)
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_432 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_433 = seg_base + (local_tid ^ 255u); let tmp_434 = smem_keys[tmp_433 * WPT + 3u]; let tmp_435 = smem_vals[tmp_433 * WPT + 3u]; let tmp_436 = keys[0] < tmp_434 || (keys[0] == tmp_434 && values[0] < tmp_435); if tmp_432 == tmp_436 { keys[0] = tmp_434; values[0] = tmp_435; } let tmp_437 = smem_keys[tmp_433 * WPT + 2u]; let tmp_438 = smem_vals[tmp_433 * WPT + 2u]; let tmp_439 = keys[1] < tmp_437 || (keys[1] == tmp_437 && values[1] < tmp_438); if tmp_432 == tmp_439 { keys[1] = tmp_437; values[1] = tmp_438; } let tmp_440 = smem_keys[tmp_433 * WPT + 1u]; let tmp_441 = smem_vals[tmp_433 * WPT + 1u]; let tmp_442 = keys[2] < tmp_440 || (keys[2] == tmp_440 && values[2] < tmp_441); if tmp_432 == tmp_442 { keys[2] = tmp_440; values[2] = tmp_441; } let tmp_443 = smem_keys[tmp_433 * WPT + 0u]; let tmp_444 = smem_vals[tmp_433 * WPT + 0u]; let tmp_445 = keys[3] < tmp_443 || (keys[3] == tmp_443 && values[3] < tmp_444); if tmp_432 == tmp_445 { keys[3] = tmp_443; values[3] = tmp_444; } workgroupBarrier(); }
    ...
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_537 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_537;let tmp_538 = values[0]; values[0] = values[2]; values[2] = tmp_538; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_539 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_539;let tmp_540 = values[1]; values[1] = values[3]; values[3] = tmp_540; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_541 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_541;let tmp_542 = values[0]; values[0] = values[1]; values[1] = tmp_542; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_543 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_543;let tmp_544 = values[2]; values[2] = values[3]; values[3] = tmp_544; }
    }
