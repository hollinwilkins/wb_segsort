...
fn segsort_hybrid_sg128_n2048_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; smem_keys[tid_g * WPT + 4u] = keys[4]; smem_vals[tid_g * WPT + 4u] = values[4]; smem_keys[tid_g * WPT + 5u] = keys[5]; smem_vals[tid_g * WPT + 5u] = values[5]; smem_keys[tid_g * WPT + 6u] = keys[6]; smem_vals[tid_g * WPT + 6u] = values[6]; smem_keys[tid_g * WPT + 7u] = keys[7]; smem_vals[tid_g * WPT + 7u] = values[7]; workgroupBarrier(); let tmp_916 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_917 = seg_base + (local_tid ^ 255u); let tmp_918 = smem_keys[tmp_917 * WPT + 7u]; let tmp_919 = smem_vals[tmp_917 * WPT + 7u]; let tmp_920 = keys[0] < tmp_918 || (keys[0] == tmp_918 && values[0] < tmp_919); if tmp_916 == tmp_920 { keys[0] = tmp_918; values[0] = tmp_919; } let tmp_921 = smem_keys[tmp_917 * WPT + 6u]; let tmp_922 = smem_vals[tmp_917 * WPT + 6u]; let tmp_923 = keys[1] < tmp_921 || (keys[1] == tmp_921 && values[1] < tmp_922); if tmp_916 == tmp_923 { keys[1] = tmp_921; values[1] = tmp_922; } let tmp_924 = smem_keys[tmp_917 * WPT + 5u]; let tmp_925 = smem_vals[tmp_917 * WPT + 5u]; let tmp_926 = keys[2] < tmp_924 || (keys[2] == tmp_924 && values[2] < tmp_925); if tmp_916 == tmp_926 { keys[2] = tmp_924; values[2] = tmp_925; } let tmp_927 = smem_keys[tmp_917 * WPT + 4u]; let tmp_928 = smem_vals[tmp_917 * WPT + 4u]; let tmp_929 = keys[3] < tmp_927 || (keys[3] == tmp_927 && values[3] < tmp_928); if tmp_916 == tmp_929 { keys[3] = tmp_927; values[3] = tmp_928; } let tmp_930 = smem_keys[tmp_917 * WPT + 3u]; let tmp_931 = smem_vals[tmp_917 * WPT + 3u]; let tmp_932 = keys[4] < tmp_930 || (keys[4] == tmp_930 && values[4] < tmp_931); if tmp_916 == tmp_932 { keys[4] = tmp_930; values[4] = tmp_931; } let tmp_933 = smem_keys[tmp_917 * WPT + 2u]; let tmp_934 = smem_vals[tmp_917 * WPT + 2u]; let tmp_935 = keys[5] < tmp_933 || (keys[5] == tmp_933 && values[5] < tmp_934); if tmp_916 == tmp_935 { keys[5] = tmp_933; values[5] = tmp_934; } let tmp_936 = smem_keys[tmp_917 * WPT + 1u]; let tmp_937 = smem_vals[tmp_917 * WPT + 1u]; let tmp_938 = keys[6] < tmp_936 || (keys[6] == tmp_936 && values[6] < tmp_937); if tmp_916 == tmp_938 { keys[6] = tmp_936; values[6] = tmp_937; } let tmp_939 = smem_keys[tmp_917 * WPT + 0u]; let tmp_940 = smem_vals[tmp_917 * WPT + 0u]; let tmp_941 = keys[7] < tmp_939 || (keys[7] == tmp_939 && values[7] < tmp_940); if tmp_916 == tmp_941 { keys[7] = tmp_939; values[7] = tmp_940; } workgroupBarrier(); }
    ...
    if keys[0] > keys[4] || (keys[0] == keys[4] && values[0] > values[4]) {
    // swap(0,4) 
    { let tmp_1117 = keys[0]; keys[0] = keys[4]; keys[4] = tmp_1117;let tmp_1118 = values[0]; values[0] = values[4]; values[4] = tmp_1118; }
    }
    // cmp_swap(1,5)
    if keys[1] > keys[5] || (keys[1] == keys[5] && values[1] > values[5]) {
    // swap(1,5) 
    { let tmp_1119 = keys[1]; keys[1] = keys[5]; keys[5] = tmp_1119;let tmp_1120 = values[1]; values[1] = values[5]; values[5] = tmp_1120; }
    }
    // cmp_swap(2,6)
    if keys[2] > keys[6] || (keys[2] == keys[6] && values[2] > values[6]) {
    // swap(2,6) 
    { let tmp_1121 = keys[2]; keys[2] = keys[6]; keys[6] = tmp_1121;let tmp_1122 = values[2]; values[2] = values[6]; values[6] = tmp_1122; }
    }
    // cmp_swap(3,7)
    if keys[3] > keys[7] || (keys[3] == keys[7] && values[3] > values[7]) {
    // swap(3,7) 
    { let tmp_1123 = keys[3]; keys[3] = keys[7]; keys[7] = tmp_1123;let tmp_1124 = values[3]; values[3] = values[7]; values[7] = tmp_1124; }
    }
    // exch_local(2,8) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_1125 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_1125;let tmp_1126 = values[0]; values[0] = values[2]; values[2] = tmp_1126; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_1127 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_1127;let tmp_1128 = values[1]; values[1] = values[3]; values[3] = tmp_1128; }
    }
    // cmp_swap(4,6)
    if keys[4] > keys[6] || (keys[4] == keys[6] && values[4] > values[6]) {
    // swap(4,6) 
    { let tmp_1129 = keys[4]; keys[4] = keys[6]; keys[6] = tmp_1129;let tmp_1130 = values[4]; values[4] = values[6]; values[6] = tmp_1130; }
    }
    // cmp_swap(5,7)
    if keys[5] > keys[7] || (keys[5] == keys[7] && values[5] > values[7]) {
    // swap(5,7) 
    { let tmp_1131 = keys[5]; keys[5] = keys[7]; keys[7] = tmp_1131;let tmp_1132 = values[5]; values[5] = values[7]; values[7] = tmp_1132; }
    }
    // exch_local(1,8) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_1133 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_1133;let tmp_1134 = values[0]; values[0] = values[1]; values[1] = tmp_1134; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_1135 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_1135;let tmp_1136 = values[2]; values[2] = values[3]; values[3] = tmp_1136; }
    }
    // cmp_swap(4,5)
    if keys[4] > keys[5] || (keys[4] == keys[5] && values[4] > values[5]) {
    // swap(4,5) 
    { let tmp_1137 = keys[4]; keys[4] = keys[5]; keys[5] = tmp_1137;let tmp_1138 = values[4]; values[4] = values[5]; values[5] = tmp_1138; }
    }
    // cmp_swap(6,7)
    if keys[6] > keys[7] || (keys[6] == keys[7] && values[6] > values[7]) {
    // swap(6,7) 
    { let tmp_1139 = keys[6]; keys[6] = keys[7]; keys[7] = tmp_1139;let tmp_1140 = values[6]; values[6] = values[7]; values[7] = tmp_1140; }
    }
