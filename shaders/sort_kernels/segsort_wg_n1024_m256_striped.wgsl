...
fn segsort_wg_n1024_m256_striped(
    ...
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_262 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_263 = seg_base + (local_tid ^ 63u); let tmp_264 = smem_keys[tmp_263 * WPT + 3u]; let tmp_265 = smem_vals[tmp_263 * WPT + 3u]; let tmp_266 = keys[0] < tmp_264 || (keys[0] == tmp_264 && values[0] < tmp_265); if tmp_262 == tmp_266 { keys[0] = tmp_264; values[0] = tmp_265; } let tmp_267 = smem_keys[tmp_263 * WPT + 2u]; let tmp_268 = smem_vals[tmp_263 * WPT + 2u]; let tmp_269 = keys[1] < tmp_267 || (keys[1] == tmp_267 && values[1] < tmp_268); if tmp_262 == tmp_269 { keys[1] = tmp_267; values[1] = tmp_268; } let tmp_270 = smem_keys[tmp_263 * WPT + 1u]; let tmp_271 = smem_vals[tmp_263 * WPT + 1u]; let tmp_272 = keys[2] < tmp_270 || (keys[2] == tmp_270 && values[2] < tmp_271); if tmp_262 == tmp_272 { keys[2] = tmp_270; values[2] = tmp_271; } let tmp_273 = smem_keys[tmp_263 * WPT + 0u]; let tmp_274 = smem_vals[tmp_263 * WPT + 0u]; let tmp_275 = keys[3] < tmp_273 || (keys[3] == tmp_273 && values[3] < tmp_274); if tmp_262 == tmp_275 { keys[3] = tmp_273; values[3] = tmp_274; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_276 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_277 = seg_base + (local_tid ^ 16u); let tmp_278 = smem_keys[tmp_277 * WPT + 0u]; let tmp_279 = smem_vals[tmp_277 * WPT + 0u]; let tmp_280 = keys[0] < tmp_278 || (keys[0] == tmp_278 && values[0] < tmp_279); if tmp_276 == tmp_280 { keys[0] = tmp_278; values[0] = tmp_279; } let tmp_281 = smem_keys[tmp_277 * WPT + 1u]; let tmp_282 = smem_vals[tmp_277 * WPT + 1u]; let tmp_283 = keys[1] < tmp_281 || (keys[1] == tmp_281 && values[1] < tmp_282); if tmp_276 == tmp_283 { keys[1] = tmp_281; values[1] = tmp_282; } let tmp_284 = smem_keys[tmp_277 * WPT + 2u]; let tmp_285 = smem_vals[tmp_277 * WPT + 2u]; let tmp_286 = keys[2] < tmp_284 || (keys[2] == tmp_284 && values[2] < tmp_285); if tmp_276 == tmp_286 { keys[2] = tmp_284; values[2] = tmp_285; } let tmp_287 = smem_keys[tmp_277 * WPT + 3u]; let tmp_288 = smem_vals[tmp_277 * WPT + 3u]; let tmp_289 = keys[3] < tmp_287 || (keys[3] == tmp_287 && values[3] < tmp_288); if tmp_276 == tmp_289 { keys[3] = tmp_287; values[3] = tmp_288; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_290 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_291 = seg_base + (local_tid ^ 8u); let tmp_292 = smem_keys[tmp_291 * WPT + 0u]; let tmp_293 = smem_vals[tmp_291 * WPT + 0u]; let tmp_294 = keys[0] < tmp_292 || (keys[0] == tmp_292 && values[0] < tmp_293); if tmp_290 == tmp_294 { keys[0] = tmp_292; values[0] = tmp_293; } let tmp_295 = smem_keys[tmp_291 * WPT + 1u]; let tmp_296 = smem_vals[tmp_291 * WPT + 1u]; let tmp_297 = keys[1] < tmp_295 || (keys[1] == tmp_295 && values[1] < tmp_296); if tmp_290 == tmp_297 { keys[1] = tmp_295; values[1] = tmp_296; } let tmp_298 = smem_keys[tmp_291 * WPT + 2u]; let tmp_299 = smem_vals[tmp_291 * WPT + 2u]; let tmp_300 = keys[2] < tmp_298 || (keys[2] == tmp_298 && values[2] < tmp_299); if tmp_290 == tmp_300 { keys[2] = tmp_298; values[2] = tmp_299; } let tmp_301 = smem_keys[tmp_291 * WPT + 3u]; let tmp_302 = smem_vals[tmp_291 * WPT + 3u]; let tmp_303 = keys[3] < tmp_301 || (keys[3] == tmp_301 && values[3] < tmp_302); if tmp_290 == tmp_303 { keys[3] = tmp_301; values[3] = tmp_302; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_304 = extractBits(local_tid, 2u, 1u) != 0u; let tmp_305 = seg_base + (local_tid ^ 4u); let tmp_306 = smem_keys[tmp_305 * WPT + 0u]; let tmp_307 = smem_vals[tmp_305 * WPT + 0u]; let tmp_308 = keys[0] < tmp_306 || (keys[0] == tmp_306 && values[0] < tmp_307); if tmp_304 == tmp_308 { keys[0] = tmp_306; values[0] = tmp_307; } let tmp_309 = smem_keys[tmp_305 * WPT + 1u]; let tmp_310 = smem_vals[tmp_305 * WPT + 1u]; let tmp_311 = keys[1] < tmp_309 || (keys[1] == tmp_309 && values[1] < tmp_310); if tmp_304 == tmp_311 { keys[1] = tmp_309; values[1] = tmp_310; } let tmp_312 = smem_keys[tmp_305 * WPT + 2u]; let tmp_313 = smem_vals[tmp_305 * WPT + 2u]; let tmp_314 = keys[2] < tmp_312 || (keys[2] == tmp_312 && values[2] < tmp_313); if tmp_304 == tmp_314 { keys[2] = tmp_312; values[2] = tmp_313; } let tmp_315 = smem_keys[tmp_305 * WPT + 3u]; let tmp_316 = smem_vals[tmp_305 * WPT + 3u]; let tmp_317 = keys[3] < tmp_315 || (keys[3] == tmp_315 && values[3] < tmp_316); if tmp_304 == tmp_317 { keys[3] = tmp_315; values[3] = tmp_316; } workgroupBarrier(); }
    // exch_paral(tmask:2,swbit:1,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_318 = extractBits(local_tid, 1u, 1u) != 0u; let tmp_319 = seg_base + (local_tid ^ 2u); let tmp_320 = smem_keys[tmp_319 * WPT + 0u]; let tmp_321 = smem_vals[tmp_319 * WPT + 0u]; let tmp_322 = keys[0] < tmp_320 || (keys[0] == tmp_320 && values[0] < tmp_321); if tmp_318 == tmp_322 { keys[0] = tmp_320; values[0] = tmp_321; } let tmp_323 = smem_keys[tmp_319 * WPT + 1u]; let tmp_324 = smem_vals[tmp_319 * WPT + 1u]; let tmp_325 = keys[1] < tmp_323 || (keys[1] == tmp_323 && values[1] < tmp_324); if tmp_318 == tmp_325 { keys[1] = tmp_323; values[1] = tmp_324; } let tmp_326 = smem_keys[tmp_319 * WPT + 2u]; let tmp_327 = smem_vals[tmp_319 * WPT + 2u]; let tmp_328 = keys[2] < tmp_326 || (keys[2] == tmp_326 && values[2] < tmp_327); if tmp_318 == tmp_328 { keys[2] = tmp_326; values[2] = tmp_327; } let tmp_329 = smem_keys[tmp_319 * WPT + 3u]; let tmp_330 = smem_vals[tmp_319 * WPT + 3u]; let tmp_331 = keys[3] < tmp_329 || (keys[3] == tmp_329 && values[3] < tmp_330); if tmp_318 == tmp_331 { keys[3] = tmp_329; values[3] = tmp_330; } workgroupBarrier(); }
    // exch_paral(tmask:1,swbit:0,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_332 = extractBits(local_tid, 0u, 1u) != 0u; let tmp_333 = seg_base + (local_tid ^ 1u); let tmp_334 = smem_keys[tmp_333 * WPT + 0u]; let tmp_335 = smem_vals[tmp_333 * WPT + 0u]; let tmp_336 = keys[0] < tmp_334 || (keys[0] == tmp_334 && values[0] < tmp_335); if tmp_332 == tmp_336 { keys[0] = tmp_334; values[0] = tmp_335; } let tmp_337 = smem_keys[tmp_333 * WPT + 1u]; let tmp_338 = smem_vals[tmp_333 * WPT + 1u]; let tmp_339 = keys[1] < tmp_337 || (keys[1] == tmp_337 && values[1] < tmp_338); if tmp_332 == tmp_339 { keys[1] = tmp_337; values[1] = tmp_338; } let tmp_340 = smem_keys[tmp_333 * WPT + 2u]; let tmp_341 = smem_vals[tmp_333 * WPT + 2u]; let tmp_342 = keys[2] < tmp_340 || (keys[2] == tmp_340 && values[2] < tmp_341); if tmp_332 == tmp_342 { keys[2] = tmp_340; values[2] = tmp_341; } let tmp_343 = smem_keys[tmp_333 * WPT + 3u]; let tmp_344 = smem_vals[tmp_333 * WPT + 3u]; let tmp_345 = keys[3] < tmp_343 || (keys[3] == tmp_343 && values[3] < tmp_344); if tmp_332 == tmp_345 { keys[3] = tmp_343; values[3] = tmp_344; } workgroupBarrier(); }
    // exch_local(2,4) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_346 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_346;let tmp_347 = values[0]; values[0] = values[2]; values[2] = tmp_347; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_348 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_348;let tmp_349 = values[1]; values[1] = values[3]; values[3] = tmp_349; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_350 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_350;let tmp_351 = values[0]; values[0] = values[1]; values[1] = tmp_351; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_352 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_352;let tmp_353 = values[2]; values[2] = values[3]; values[3] = tmp_353; }
    }
    // exch_intxn(tmask:127,swbit:6,wpt:4)
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_354 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_355 = seg_base + (local_tid ^ 127u); let tmp_356 = smem_keys[tmp_355 * WPT + 3u]; let tmp_357 = smem_vals[tmp_355 * WPT + 3u]; let tmp_358 = keys[0] < tmp_356 || (keys[0] == tmp_356 && values[0] < tmp_357); if tmp_354 == tmp_358 { keys[0] = tmp_356; values[0] = tmp_357; } let tmp_359 = smem_keys[tmp_355 * WPT + 2u]; let tmp_360 = smem_vals[tmp_355 * WPT + 2u]; let tmp_361 = keys[1] < tmp_359 || (keys[1] == tmp_359 && values[1] < tmp_360); if tmp_354 == tmp_361 { keys[1] = tmp_359; values[1] = tmp_360; } let tmp_362 = smem_keys[tmp_355 * WPT + 1u]; let tmp_363 = smem_vals[tmp_355 * WPT + 1u]; let tmp_364 = keys[2] < tmp_362 || (keys[2] == tmp_362 && values[2] < tmp_363); if tmp_354 == tmp_364 { keys[2] = tmp_362; values[2] = tmp_363; } let tmp_365 = smem_keys[tmp_355 * WPT + 0u]; let tmp_366 = smem_vals[tmp_355 * WPT + 0u]; let tmp_367 = keys[3] < tmp_365 || (keys[3] == tmp_365 && values[3] < tmp_366); if tmp_354 == tmp_367 { keys[3] = tmp_365; values[3] = tmp_366; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_368 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_369 = seg_base + (local_tid ^ 32u); let tmp_370 = smem_keys[tmp_369 * WPT + 0u]; let tmp_371 = smem_vals[tmp_369 * WPT + 0u]; let tmp_372 = keys[0] < tmp_370 || (keys[0] == tmp_370 && values[0] < tmp_371); if tmp_368 == tmp_372 { keys[0] = tmp_370; values[0] = tmp_371; } let tmp_373 = smem_keys[tmp_369 * WPT + 1u]; let tmp_374 = smem_vals[tmp_369 * WPT + 1u]; let tmp_375 = keys[1] < tmp_373 || (keys[1] == tmp_373 && values[1] < tmp_374); if tmp_368 == tmp_375 { keys[1] = tmp_373; values[1] = tmp_374; } let tmp_376 = smem_keys[tmp_369 * WPT + 2u]; let tmp_377 = smem_vals[tmp_369 * WPT + 2u]; let tmp_378 = keys[2] < tmp_376 || (keys[2] == tmp_376 && values[2] < tmp_377); if tmp_368 == tmp_378 { keys[2] = tmp_376; values[2] = tmp_377; } let tmp_379 = smem_keys[tmp_369 * WPT + 3u]; let tmp_380 = smem_vals[tmp_369 * WPT + 3u]; let tmp_381 = keys[3] < tmp_379 || (keys[3] == tmp_379 && values[3] < tmp_380); if tmp_368 == tmp_381 { keys[3] = tmp_379; values[3] = tmp_380; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_382 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_383 = seg_base + (local_tid ^ 16u); let tmp_384 = smem_keys[tmp_383 * WPT + 0u]; let tmp_385 = smem_vals[tmp_383 * WPT + 0u]; let tmp_386 = keys[0] < tmp_384 || (keys[0] == tmp_384 && values[0] < tmp_385); if tmp_382 == tmp_386 { keys[0] = tmp_384; values[0] = tmp_385; } let tmp_387 = smem_keys[tmp_383 * WPT + 1u]; let tmp_388 = smem_vals[tmp_383 * WPT + 1u]; let tmp_389 = keys[1] < tmp_387 || (keys[1] == tmp_387 && values[1] < tmp_388); if tmp_382 == tmp_389 { keys[1] = tmp_387; values[1] = tmp_388; } let tmp_390 = smem_keys[tmp_383 * WPT + 2u]; let tmp_391 = smem_vals[tmp_383 * WPT + 2u]; let tmp_392 = keys[2] < tmp_390 || (keys[2] == tmp_390 && values[2] < tmp_391); if tmp_382 == tmp_392 { keys[2] = tmp_390; values[2] = tmp_391; } let tmp_393 = smem_keys[tmp_383 * WPT + 3u]; let tmp_394 = smem_vals[tmp_383 * WPT + 3u]; let tmp_395 = keys[3] < tmp_393 || (keys[3] == tmp_393 && values[3] < tmp_394); if tmp_382 == tmp_395 { keys[3] = tmp_393; values[3] = tmp_394; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_396 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_397 = seg_base + (local_tid ^ 8u); let tmp_398 = smem_keys[tmp_397 * WPT + 0u]; let tmp_399 = smem_vals[tmp_397 * WPT + 0u]; let tmp_400 = keys[0] < tmp_398 || (keys[0] == tmp_398 && values[0] < tmp_399); if tmp_396 == tmp_400 { keys[0] = tmp_398; values[0] = tmp_399; } let tmp_401 = smem_keys[tmp_397 * WPT + 1u]; let tmp_402 = smem_vals[tmp_397 * WPT + 1u]; let tmp_403 = keys[1] < tmp_401 || (keys[1] == tmp_401 && values[1] < tmp_402); if tmp_396 == tmp_403 { keys[1] = tmp_401; values[1] = tmp_402; } let tmp_404 = smem_keys[tmp_397 * WPT + 2u]; let tmp_405 = smem_vals[tmp_397 * WPT + 2u]; let tmp_406 = keys[2] < tmp_404 || (keys[2] == tmp_404 && values[2] < tmp_405); if tmp_396 == tmp_406 { keys[2] = tmp_404; values[2] = tmp_405; } let tmp_407 = smem_keys[tmp_397 * WPT + 3u]; let tmp_408 = smem_vals[tmp_397 * WPT + 3u]; let tmp_409 = keys[3] < tmp_407 || (keys[3] == tmp_407 && values[3] < tmp_408); if tmp_396 == tmp_409 { keys[3] = tmp_407; values[3] = tmp_408; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_410 = extractBits(local_tid, 2u, 1u) != 0u; let tmp_411 = seg_base + (local_tid ^ 4u); let tmp_412 = smem_keys[tmp_411 * WPT + 0u]; let tmp_413 = smem_vals[tmp_411 * WPT + 0u]; let tmp_414 = keys[0] < tmp_412 || (keys[0] == tmp_412 && values[0] < tmp_413); if tmp_410 == tmp_414 { keys[0] = tmp_412; values[0] = tmp_413; } let tmp_415 = smem_keys[tmp_411 * WPT + 1u]; let tmp_416 = smem_vals[tmp_411 * WPT + 1u]; let tmp_417 = keys[1] < tmp_415 || (keys[1] == tmp_415 && values[1] < tmp_416); if tmp_410 == tmp_417 { keys[1] = tmp_415; values[1] = tmp_416; } let tmp_418 = smem_keys[tmp_411 * WPT + 2u]; let tmp_419 = smem_vals[tmp_411 * WPT + 2u]; let tmp_420 = keys[2] < tmp_418 || (keys[2] == tmp_418 && values[2] < tmp_419); if tmp_410 == tmp_420 { keys[2] = tmp_418; values[2] = tmp_419; } let tmp_421 = smem_keys[tmp_411 * WPT + 3u]; let tmp_422 = smem_vals[tmp_411 * WPT + 3u]; let tmp_423 = keys[3] < tmp_421 || (keys[3] == tmp_421 && values[3] < tmp_422); if tmp_410 == tmp_423 { keys[3] = tmp_421; values[3] = tmp_422; } workgroupBarrier(); }
    // exch_paral(tmask:2,swbit:1,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_424 = extractBits(local_tid, 1u, 1u) != 0u; let tmp_425 = seg_base + (local_tid ^ 2u); let tmp_426 = smem_keys[tmp_425 * WPT + 0u]; let tmp_427 = smem_vals[tmp_425 * WPT + 0u]; let tmp_428 = keys[0] < tmp_426 || (keys[0] == tmp_426 && values[0] < tmp_427); if tmp_424 == tmp_428 { keys[0] = tmp_426; values[0] = tmp_427; } let tmp_429 = smem_keys[tmp_425 * WPT + 1u]; let tmp_430 = smem_vals[tmp_425 * WPT + 1u]; let tmp_431 = keys[1] < tmp_429 || (keys[1] == tmp_429 && values[1] < tmp_430); if tmp_424 == tmp_431 { keys[1] = tmp_429; values[1] = tmp_430; } let tmp_432 = smem_keys[tmp_425 * WPT + 2u]; let tmp_433 = smem_vals[tmp_425 * WPT + 2u]; let tmp_434 = keys[2] < tmp_432 || (keys[2] == tmp_432 && values[2] < tmp_433); if tmp_424 == tmp_434 { keys[2] = tmp_432; values[2] = tmp_433; } let tmp_435 = smem_keys[tmp_425 * WPT + 3u]; let tmp_436 = smem_vals[tmp_425 * WPT + 3u]; let tmp_437 = keys[3] < tmp_435 || (keys[3] == tmp_435 && values[3] < tmp_436); if tmp_424 == tmp_437 { keys[3] = tmp_435; values[3] = tmp_436; } workgroupBarrier(); }
    // exch_paral(tmask:1,swbit:0,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_438 = extractBits(local_tid, 0u, 1u) != 0u; let tmp_439 = seg_base + (local_tid ^ 1u); let tmp_440 = smem_keys[tmp_439 * WPT + 0u]; let tmp_441 = smem_vals[tmp_439 * WPT + 0u]; let tmp_442 = keys[0] < tmp_440 || (keys[0] == tmp_440 && values[0] < tmp_441); if tmp_438 == tmp_442 { keys[0] = tmp_440; values[0] = tmp_441; } let tmp_443 = smem_keys[tmp_439 * WPT + 1u]; let tmp_444 = smem_vals[tmp_439 * WPT + 1u]; let tmp_445 = keys[1] < tmp_443 || (keys[1] == tmp_443 && values[1] < tmp_444); if tmp_438 == tmp_445 { keys[1] = tmp_443; values[1] = tmp_444; } let tmp_446 = smem_keys[tmp_439 * WPT + 2u]; let tmp_447 = smem_vals[tmp_439 * WPT + 2u]; let tmp_448 = keys[2] < tmp_446 || (keys[2] == tmp_446 && values[2] < tmp_447); if tmp_438 == tmp_448 { keys[2] = tmp_446; values[2] = tmp_447; } let tmp_449 = smem_keys[tmp_439 * WPT + 3u]; let tmp_450 = smem_vals[tmp_439 * WPT + 3u]; let tmp_451 = keys[3] < tmp_449 || (keys[3] == tmp_449 && values[3] < tmp_450); if tmp_438 == tmp_451 { keys[3] = tmp_449; values[3] = tmp_450; } workgroupBarrier(); }
    // exch_local(2,4) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_452 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_452;let tmp_453 = values[0]; values[0] = values[2]; values[2] = tmp_453; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_454 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_454;let tmp_455 = values[1]; values[1] = values[3]; values[3] = tmp_455; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
    // swap(0,1) 
    { let tmp_456 = keys[0]; keys[0] = keys[1]; keys[1] = tmp_456;let tmp_457 = values[0]; values[0] = values[1]; values[1] = tmp_457; }
    }
    // cmp_swap(2,3)
    if keys[2] > keys[3] || (keys[2] == keys[3] && values[2] > values[3]) {
    // swap(2,3) 
    { let tmp_458 = keys[2]; keys[2] = keys[3]; keys[3] = tmp_458;let tmp_459 = values[2]; values[2] = values[3]; values[3] = tmp_459; }
    }
    // exch_intxn(tmask:255,swbit:7,wpt:4)
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_460 = extractBits(local_tid, 7u, 1u) != 0u; let tmp_461 = seg_base + (local_tid ^ 255u); let tmp_462 = smem_keys[tmp_461 * WPT + 3u]; let tmp_463 = smem_vals[tmp_461 * WPT + 3u]; let tmp_464 = keys[0] < tmp_462 || (keys[0] == tmp_462 && values[0] < tmp_463); if tmp_460 == tmp_464 { keys[0] = tmp_462; values[0] = tmp_463; } let tmp_465 = smem_keys[tmp_461 * WPT + 2u]; let tmp_466 = smem_vals[tmp_461 * WPT + 2u]; let tmp_467 = keys[1] < tmp_465 || (keys[1] == tmp_465 && values[1] < tmp_466); if tmp_460 == tmp_467 { keys[1] = tmp_465; values[1] = tmp_466; } let tmp_468 = smem_keys[tmp_461 * WPT + 1u]; let tmp_469 = smem_vals[tmp_461 * WPT + 1u]; let tmp_470 = keys[2] < tmp_468 || (keys[2] == tmp_468 && values[2] < tmp_469); if tmp_460 == tmp_470 { keys[2] = tmp_468; values[2] = tmp_469; } let tmp_471 = smem_keys[tmp_461 * WPT + 0u]; let tmp_472 = smem_vals[tmp_461 * WPT + 0u]; let tmp_473 = keys[3] < tmp_471 || (keys[3] == tmp_471 && values[3] < tmp_472); if tmp_460 == tmp_473 { keys[3] = tmp_471; values[3] = tmp_472; } workgroupBarrier(); }
    // exch_paral(tmask:64,swbit:6,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_474 = extractBits(local_tid, 6u, 1u) != 0u; let tmp_475 = seg_base + (local_tid ^ 64u); let tmp_476 = smem_keys[tmp_475 * WPT + 0u]; let tmp_477 = smem_vals[tmp_475 * WPT + 0u]; let tmp_478 = keys[0] < tmp_476 || (keys[0] == tmp_476 && values[0] < tmp_477); if tmp_474 == tmp_478 { keys[0] = tmp_476; values[0] = tmp_477; } let tmp_479 = smem_keys[tmp_475 * WPT + 1u]; let tmp_480 = smem_vals[tmp_475 * WPT + 1u]; let tmp_481 = keys[1] < tmp_479 || (keys[1] == tmp_479 && values[1] < tmp_480); if tmp_474 == tmp_481 { keys[1] = tmp_479; values[1] = tmp_480; } let tmp_482 = smem_keys[tmp_475 * WPT + 2u]; let tmp_483 = smem_vals[tmp_475 * WPT + 2u]; let tmp_484 = keys[2] < tmp_482 || (keys[2] == tmp_482 && values[2] < tmp_483); if tmp_474 == tmp_484 { keys[2] = tmp_482; values[2] = tmp_483; } let tmp_485 = smem_keys[tmp_475 * WPT + 3u]; let tmp_486 = smem_vals[tmp_475 * WPT + 3u]; let tmp_487 = keys[3] < tmp_485 || (keys[3] == tmp_485 && values[3] < tmp_486); if tmp_474 == tmp_487 { keys[3] = tmp_485; values[3] = tmp_486; } workgroupBarrier(); }
    // exch_paral(tmask:32,swbit:5,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_488 = extractBits(local_tid, 5u, 1u) != 0u; let tmp_489 = seg_base + (local_tid ^ 32u); let tmp_490 = smem_keys[tmp_489 * WPT + 0u]; let tmp_491 = smem_vals[tmp_489 * WPT + 0u]; let tmp_492 = keys[0] < tmp_490 || (keys[0] == tmp_490 && values[0] < tmp_491); if tmp_488 == tmp_492 { keys[0] = tmp_490; values[0] = tmp_491; } let tmp_493 = smem_keys[tmp_489 * WPT + 1u]; let tmp_494 = smem_vals[tmp_489 * WPT + 1u]; let tmp_495 = keys[1] < tmp_493 || (keys[1] == tmp_493 && values[1] < tmp_494); if tmp_488 == tmp_495 { keys[1] = tmp_493; values[1] = tmp_494; } let tmp_496 = smem_keys[tmp_489 * WPT + 2u]; let tmp_497 = smem_vals[tmp_489 * WPT + 2u]; let tmp_498 = keys[2] < tmp_496 || (keys[2] == tmp_496 && values[2] < tmp_497); if tmp_488 == tmp_498 { keys[2] = tmp_496; values[2] = tmp_497; } let tmp_499 = smem_keys[tmp_489 * WPT + 3u]; let tmp_500 = smem_vals[tmp_489 * WPT + 3u]; let tmp_501 = keys[3] < tmp_499 || (keys[3] == tmp_499 && values[3] < tmp_500); if tmp_488 == tmp_501 { keys[3] = tmp_499; values[3] = tmp_500; } workgroupBarrier(); }
    // exch_paral(tmask:16,swbit:4,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_502 = extractBits(local_tid, 4u, 1u) != 0u; let tmp_503 = seg_base + (local_tid ^ 16u); let tmp_504 = smem_keys[tmp_503 * WPT + 0u]; let tmp_505 = smem_vals[tmp_503 * WPT + 0u]; let tmp_506 = keys[0] < tmp_504 || (keys[0] == tmp_504 && values[0] < tmp_505); if tmp_502 == tmp_506 { keys[0] = tmp_504; values[0] = tmp_505; } let tmp_507 = smem_keys[tmp_503 * WPT + 1u]; let tmp_508 = smem_vals[tmp_503 * WPT + 1u]; let tmp_509 = keys[1] < tmp_507 || (keys[1] == tmp_507 && values[1] < tmp_508); if tmp_502 == tmp_509 { keys[1] = tmp_507; values[1] = tmp_508; } let tmp_510 = smem_keys[tmp_503 * WPT + 2u]; let tmp_511 = smem_vals[tmp_503 * WPT + 2u]; let tmp_512 = keys[2] < tmp_510 || (keys[2] == tmp_510 && values[2] < tmp_511); if tmp_502 == tmp_512 { keys[2] = tmp_510; values[2] = tmp_511; } let tmp_513 = smem_keys[tmp_503 * WPT + 3u]; let tmp_514 = smem_vals[tmp_503 * WPT + 3u]; let tmp_515 = keys[3] < tmp_513 || (keys[3] == tmp_513 && values[3] < tmp_514); if tmp_502 == tmp_515 { keys[3] = tmp_513; values[3] = tmp_514; } workgroupBarrier(); }
    // exch_paral(tmask:8,swbit:3,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_516 = extractBits(local_tid, 3u, 1u) != 0u; let tmp_517 = seg_base + (local_tid ^ 8u); let tmp_518 = smem_keys[tmp_517 * WPT + 0u]; let tmp_519 = smem_vals[tmp_517 * WPT + 0u]; let tmp_520 = keys[0] < tmp_518 || (keys[0] == tmp_518 && values[0] < tmp_519); if tmp_516 == tmp_520 { keys[0] = tmp_518; values[0] = tmp_519; } let tmp_521 = smem_keys[tmp_517 * WPT + 1u]; let tmp_522 = smem_vals[tmp_517 * WPT + 1u]; let tmp_523 = keys[1] < tmp_521 || (keys[1] == tmp_521 && values[1] < tmp_522); if tmp_516 == tmp_523 { keys[1] = tmp_521; values[1] = tmp_522; } let tmp_524 = smem_keys[tmp_517 * WPT + 2u]; let tmp_525 = smem_vals[tmp_517 * WPT + 2u]; let tmp_526 = keys[2] < tmp_524 || (keys[2] == tmp_524 && values[2] < tmp_525); if tmp_516 == tmp_526 { keys[2] = tmp_524; values[2] = tmp_525; } let tmp_527 = smem_keys[tmp_517 * WPT + 3u]; let tmp_528 = smem_vals[tmp_517 * WPT + 3u]; let tmp_529 = keys[3] < tmp_527 || (keys[3] == tmp_527 && values[3] < tmp_528); if tmp_516 == tmp_529 { keys[3] = tmp_527; values[3] = tmp_528; } workgroupBarrier(); }
    // exch_paral(tmask:4,swbit:2,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_530 = extractBits(local_tid, 2u, 1u) != 0u; let tmp_531 = seg_base + (local_tid ^ 4u); let tmp_532 = smem_keys[tmp_531 * WPT + 0u]; let tmp_533 = smem_vals[tmp_531 * WPT + 0u]; let tmp_534 = keys[0] < tmp_532 || (keys[0] == tmp_532 && values[0] < tmp_533); if tmp_530 == tmp_534 { keys[0] = tmp_532; values[0] = tmp_533; } let tmp_535 = smem_keys[tmp_531 * WPT + 1u]; let tmp_536 = smem_vals[tmp_531 * WPT + 1u]; let tmp_537 = keys[1] < tmp_535 || (keys[1] == tmp_535 && values[1] < tmp_536); if tmp_530 == tmp_537 { keys[1] = tmp_535; values[1] = tmp_536; } let tmp_538 = smem_keys[tmp_531 * WPT + 2u]; let tmp_539 = smem_vals[tmp_531 * WPT + 2u]; let tmp_540 = keys[2] < tmp_538 || (keys[2] == tmp_538 && values[2] < tmp_539); if tmp_530 == tmp_540 { keys[2] = tmp_538; values[2] = tmp_539; } let tmp_541 = smem_keys[tmp_531 * WPT + 3u]; let tmp_542 = smem_vals[tmp_531 * WPT + 3u]; let tmp_543 = keys[3] < tmp_541 || (keys[3] == tmp_541 && values[3] < tmp_542); if tmp_530 == tmp_543 { keys[3] = tmp_541; values[3] = tmp_542; } workgroupBarrier(); }
    // exch_paral(tmask:2,swbit:1,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_544 = extractBits(local_tid, 1u, 1u) != 0u; let tmp_545 = seg_base + (local_tid ^ 2u); let tmp_546 = smem_keys[tmp_545 * WPT + 0u]; let tmp_547 = smem_vals[tmp_545 * WPT + 0u]; let tmp_548 = keys[0] < tmp_546 || (keys[0] == tmp_546 && values[0] < tmp_547); if tmp_544 == tmp_548 { keys[0] = tmp_546; values[0] = tmp_547; } let tmp_549 = smem_keys[tmp_545 * WPT + 1u]; let tmp_550 = smem_vals[tmp_545 * WPT + 1u]; let tmp_551 = keys[1] < tmp_549 || (keys[1] == tmp_549 && values[1] < tmp_550); if tmp_544 == tmp_551 { keys[1] = tmp_549; values[1] = tmp_550; } let tmp_552 = smem_keys[tmp_545 * WPT + 2u]; let tmp_553 = smem_vals[tmp_545 * WPT + 2u]; let tmp_554 = keys[2] < tmp_552 || (keys[2] == tmp_552 && values[2] < tmp_553); if tmp_544 == tmp_554 { keys[2] = tmp_552; values[2] = tmp_553; } let tmp_555 = smem_keys[tmp_545 * WPT + 3u]; let tmp_556 = smem_vals[tmp_545 * WPT + 3u]; let tmp_557 = keys[3] < tmp_555 || (keys[3] == tmp_555 && values[3] < tmp_556); if tmp_544 == tmp_557 { keys[3] = tmp_555; values[3] = tmp_556; } workgroupBarrier(); }
    // exch_paral(tmask:1,swbit:0,wpt:4) 
    { smem_keys[tid_g * WPT + 0u] = keys[0]; smem_vals[tid_g * WPT + 0u] = values[0]; smem_keys[tid_g * WPT + 1u] = keys[1]; smem_vals[tid_g * WPT + 1u] = values[1]; smem_keys[tid_g * WPT + 2u] = keys[2]; smem_vals[tid_g * WPT + 2u] = values[2]; smem_keys[tid_g * WPT + 3u] = keys[3]; smem_vals[tid_g * WPT + 3u] = values[3]; workgroupBarrier(); let tmp_558 = extractBits(local_tid, 0u, 1u) != 0u; let tmp_559 = seg_base + (local_tid ^ 1u); let tmp_560 = smem_keys[tmp_559 * WPT + 0u]; let tmp_561 = smem_vals[tmp_559 * WPT + 0u]; let tmp_562 = keys[0] < tmp_560 || (keys[0] == tmp_560 && values[0] < tmp_561); if tmp_558 == tmp_562 { keys[0] = tmp_560; values[0] = tmp_561; } let tmp_563 = smem_keys[tmp_559 * WPT + 1u]; let tmp_564 = smem_vals[tmp_559 * WPT + 1u]; let tmp_565 = keys[1] < tmp_563 || (keys[1] == tmp_563 && values[1] < tmp_564); if tmp_558 == tmp_565 { keys[1] = tmp_563; values[1] = tmp_564; } let tmp_566 = smem_keys[tmp_559 * WPT + 2u]; let tmp_567 = smem_vals[tmp_559 * WPT + 2u]; let tmp_568 = keys[2] < tmp_566 || (keys[2] == tmp_566 && values[2] < tmp_567); if tmp_558 == tmp_568 { keys[2] = tmp_566; values[2] = tmp_567; } let tmp_569 = smem_keys[tmp_559 * WPT + 3u]; let tmp_570 = smem_vals[tmp_559 * WPT + 3u]; let tmp_571 = keys[3] < tmp_569 || (keys[3] == tmp_569 && values[3] < tmp_570); if tmp_558 == tmp_571 { keys[3] = tmp_569; values[3] = tmp_570; } workgroupBarrier(); }
    // exch_local(2,4) 
    // cmp_swap(0,2)
    if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
    // swap(0,2) 
    { let tmp_572 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_572;let tmp_573 = values[0]; values[0] = values[2]; values[2] = tmp_573; }
    }
    // cmp_swap(1,3)
    if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
    // swap(1,3) 
    { let tmp_574 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_574;let tmp_575 = values[1]; values[1] = values[3]; values[3] = tmp_575; }
    }
    // exch_local(1,4) 
    // cmp_swap(0,1)
    if keys[0] > keys[1] || (keys[0] == keys[1] && values[0] > values[1]) {
