...
fn segsort_wg_n1024_m256_striped(
    ...
    // exch_intxn(tmask:255,swbit:7,wpt:4)
    {
        // _exch_workgroup([(0, 3), (1, 2), (2, 1), (3, 0)],255,7)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_460 = extractBits(local_tid, 7u, 1u) != 0u;
            let tmp_461 = seg_base + (local_tid ^ 255u);
            let tmp_462 = smem_keys[tmp_461 * WPT + 3u];
            let tmp_463 = smem_vals[tmp_461 * WPT + 3u];
            let tmp_464 = keys[0] < tmp_462 || (keys[0] == tmp_462 && values[0] < tmp_463);
            if tmp_460 == tmp_464 { keys[0] = tmp_462; values[0] = tmp_463; }
            let tmp_465 = smem_keys[tmp_461 * WPT + 2u];
            let tmp_466 = smem_vals[tmp_461 * WPT + 2u];
            let tmp_467 = keys[1] < tmp_465 || (keys[1] == tmp_465 && values[1] < tmp_466);
            if tmp_460 == tmp_467 { keys[1] = tmp_465; values[1] = tmp_466; }
            let tmp_468 = smem_keys[tmp_461 * WPT + 1u];
            let tmp_469 = smem_vals[tmp_461 * WPT + 1u];
            let tmp_470 = keys[2] < tmp_468 || (keys[2] == tmp_468 && values[2] < tmp_469);
            if tmp_460 == tmp_470 { keys[2] = tmp_468; values[2] = tmp_469; }
            let tmp_471 = smem_keys[tmp_461 * WPT + 0u];
            let tmp_472 = smem_vals[tmp_461 * WPT + 0u];
            let tmp_473 = keys[3] < tmp_471 || (keys[3] == tmp_471 && values[3] < tmp_472);
            if tmp_460 == tmp_473 { keys[3] = tmp_471; values[3] = tmp_472; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:64,swbit:6,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],64,6)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_474 = extractBits(local_tid, 6u, 1u) != 0u;
            let tmp_475 = seg_base + (local_tid ^ 64u);
            let tmp_476 = smem_keys[tmp_475 * WPT + 0u];
            let tmp_477 = smem_vals[tmp_475 * WPT + 0u];
            let tmp_478 = keys[0] < tmp_476 || (keys[0] == tmp_476 && values[0] < tmp_477);
            if tmp_474 == tmp_478 { keys[0] = tmp_476; values[0] = tmp_477; }
            let tmp_479 = smem_keys[tmp_475 * WPT + 1u];
            let tmp_480 = smem_vals[tmp_475 * WPT + 1u];
            let tmp_481 = keys[1] < tmp_479 || (keys[1] == tmp_479 && values[1] < tmp_480);
            if tmp_474 == tmp_481 { keys[1] = tmp_479; values[1] = tmp_480; }
            let tmp_482 = smem_keys[tmp_475 * WPT + 2u];
            let tmp_483 = smem_vals[tmp_475 * WPT + 2u];
            let tmp_484 = keys[2] < tmp_482 || (keys[2] == tmp_482 && values[2] < tmp_483);
            if tmp_474 == tmp_484 { keys[2] = tmp_482; values[2] = tmp_483; }
            let tmp_485 = smem_keys[tmp_475 * WPT + 3u];
            let tmp_486 = smem_vals[tmp_475 * WPT + 3u];
            let tmp_487 = keys[3] < tmp_485 || (keys[3] == tmp_485 && values[3] < tmp_486);
            if tmp_474 == tmp_487 { keys[3] = tmp_485; values[3] = tmp_486; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:32,swbit:5,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],32,5)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_488 = extractBits(local_tid, 5u, 1u) != 0u;
            let tmp_489 = seg_base + (local_tid ^ 32u);
    ...
    // exch_paral(tmask:16,swbit:4,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],16,4)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_502 = extractBits(local_tid, 4u, 1u) != 0u;
            let tmp_503 = seg_base + (local_tid ^ 16u);
            let tmp_504 = smem_keys[tmp_503 * WPT + 0u];
            let tmp_505 = smem_vals[tmp_503 * WPT + 0u];
            let tmp_506 = keys[0] < tmp_504 || (keys[0] == tmp_504 && values[0] < tmp_505);
            if tmp_502 == tmp_506 { keys[0] = tmp_504; values[0] = tmp_505; }
            let tmp_507 = smem_keys[tmp_503 * WPT + 1u];
            let tmp_508 = smem_vals[tmp_503 * WPT + 1u];
            let tmp_509 = keys[1] < tmp_507 || (keys[1] == tmp_507 && values[1] < tmp_508);
            if tmp_502 == tmp_509 { keys[1] = tmp_507; values[1] = tmp_508; }
            let tmp_510 = smem_keys[tmp_503 * WPT + 2u];
            let tmp_511 = smem_vals[tmp_503 * WPT + 2u];
            let tmp_512 = keys[2] < tmp_510 || (keys[2] == tmp_510 && values[2] < tmp_511);
            if tmp_502 == tmp_512 { keys[2] = tmp_510; values[2] = tmp_511; }
            let tmp_513 = smem_keys[tmp_503 * WPT + 3u];
            let tmp_514 = smem_vals[tmp_503 * WPT + 3u];
            let tmp_515 = keys[3] < tmp_513 || (keys[3] == tmp_513 && values[3] < tmp_514);
            if tmp_502 == tmp_515 { keys[3] = tmp_513; values[3] = tmp_514; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:8,swbit:3,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],8,3)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_516 = extractBits(local_tid, 3u, 1u) != 0u;
            let tmp_517 = seg_base + (local_tid ^ 8u);
            let tmp_518 = smem_keys[tmp_517 * WPT + 0u];
            let tmp_519 = smem_vals[tmp_517 * WPT + 0u];
            let tmp_520 = keys[0] < tmp_518 || (keys[0] == tmp_518 && values[0] < tmp_519);
            if tmp_516 == tmp_520 { keys[0] = tmp_518; values[0] = tmp_519; }
            let tmp_521 = smem_keys[tmp_517 * WPT + 1u];
            let tmp_522 = smem_vals[tmp_517 * WPT + 1u];
            let tmp_523 = keys[1] < tmp_521 || (keys[1] == tmp_521 && values[1] < tmp_522);
            if tmp_516 == tmp_523 { keys[1] = tmp_521; values[1] = tmp_522; }
            let tmp_524 = smem_keys[tmp_517 * WPT + 2u];
            let tmp_525 = smem_vals[tmp_517 * WPT + 2u];
            let tmp_526 = keys[2] < tmp_524 || (keys[2] == tmp_524 && values[2] < tmp_525);
            if tmp_516 == tmp_526 { keys[2] = tmp_524; values[2] = tmp_525; }
            let tmp_527 = smem_keys[tmp_517 * WPT + 3u];
            let tmp_528 = smem_vals[tmp_517 * WPT + 3u];
            let tmp_529 = keys[3] < tmp_527 || (keys[3] == tmp_527 && values[3] < tmp_528);
            if tmp_516 == tmp_529 { keys[3] = tmp_527; values[3] = tmp_528; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:4,swbit:2,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],4,2)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_530 = extractBits(local_tid, 2u, 1u) != 0u;
            let tmp_531 = seg_base + (local_tid ^ 4u);
            let tmp_532 = smem_keys[tmp_531 * WPT + 0u];
            let tmp_533 = smem_vals[tmp_531 * WPT + 0u];
            let tmp_534 = keys[0] < tmp_532 || (keys[0] == tmp_532 && values[0] < tmp_533);
            if tmp_530 == tmp_534 { keys[0] = tmp_532; values[0] = tmp_533; }
            let tmp_535 = smem_keys[tmp_531 * WPT + 1u];
            let tmp_536 = smem_vals[tmp_531 * WPT + 1u];
            let tmp_537 = keys[1] < tmp_535 || (keys[1] == tmp_535 && values[1] < tmp_536);
            if tmp_530 == tmp_537 { keys[1] = tmp_535; values[1] = tmp_536; }
            let tmp_538 = smem_keys[tmp_531 * WPT + 2u];
            let tmp_539 = smem_vals[tmp_531 * WPT + 2u];
            let tmp_540 = keys[2] < tmp_538 || (keys[2] == tmp_538 && values[2] < tmp_539);
            if tmp_530 == tmp_540 { keys[2] = tmp_538; values[2] = tmp_539; }
            let tmp_541 = smem_keys[tmp_531 * WPT + 3u];
            let tmp_542 = smem_vals[tmp_531 * WPT + 3u];
            let tmp_543 = keys[3] < tmp_541 || (keys[3] == tmp_541 && values[3] < tmp_542);
            if tmp_530 == tmp_543 { keys[3] = tmp_541; values[3] = tmp_542; }
            workgroupBarrier();
        }
    }
    ...
    // exch_paral(tmask:2,swbit:1,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],2,1)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_544 = extractBits(local_tid, 1u, 1u) != 0u;
            let tmp_545 = seg_base + (local_tid ^ 2u);
            let tmp_546 = smem_keys[tmp_545 * WPT + 0u];
            let tmp_547 = smem_vals[tmp_545 * WPT + 0u];
            let tmp_548 = keys[0] < tmp_546 || (keys[0] == tmp_546 && values[0] < tmp_547);
            if tmp_544 == tmp_548 { keys[0] = tmp_546; values[0] = tmp_547; }
            let tmp_549 = smem_keys[tmp_545 * WPT + 1u];
            let tmp_550 = smem_vals[tmp_545 * WPT + 1u];
            let tmp_551 = keys[1] < tmp_549 || (keys[1] == tmp_549 && values[1] < tmp_550);
            if tmp_544 == tmp_551 { keys[1] = tmp_549; values[1] = tmp_550; }
            let tmp_552 = smem_keys[tmp_545 * WPT + 2u];
            let tmp_553 = smem_vals[tmp_545 * WPT + 2u];
            let tmp_554 = keys[2] < tmp_552 || (keys[2] == tmp_552 && values[2] < tmp_553);
            if tmp_544 == tmp_554 { keys[2] = tmp_552; values[2] = tmp_553; }
            let tmp_555 = smem_keys[tmp_545 * WPT + 3u];
            let tmp_556 = smem_vals[tmp_545 * WPT + 3u];
    ...
    // exch_paral(tmask:1,swbit:0,wpt:4)
    {
        // _exch_workgroup([(0, 0), (1, 1), (2, 2), (3, 3)],1,0)
        {
            ...
            smem_vals[tid_g * WPT + 3u] = values[3];
            workgroupBarrier();
            let tmp_558 = extractBits(local_tid, 0u, 1u) != 0u;
            let tmp_559 = seg_base + (local_tid ^ 1u);
            let tmp_560 = smem_keys[tmp_559 * WPT + 0u];
            let tmp_561 = smem_vals[tmp_559 * WPT + 0u];
            let tmp_562 = keys[0] < tmp_560 || (keys[0] == tmp_560 && values[0] < tmp_561);
            if tmp_558 == tmp_562 { keys[0] = tmp_560; values[0] = tmp_561; }
            let tmp_563 = smem_keys[tmp_559 * WPT + 1u];
            let tmp_564 = smem_vals[tmp_559 * WPT + 1u];
            let tmp_565 = keys[1] < tmp_563 || (keys[1] == tmp_563 && values[1] < tmp_564);
            if tmp_558 == tmp_565 { keys[1] = tmp_563; values[1] = tmp_564; }
            let tmp_566 = smem_keys[tmp_559 * WPT + 2u];
            let tmp_567 = smem_vals[tmp_559 * WPT + 2u];
            let tmp_568 = keys[2] < tmp_566 || (keys[2] == tmp_566 && values[2] < tmp_567);
            if tmp_558 == tmp_568 { keys[2] = tmp_566; values[2] = tmp_567; }
            let tmp_569 = smem_keys[tmp_559 * WPT + 3u];
            let tmp_570 = smem_vals[tmp_559 * WPT + 3u];
    ...
    // exch_local(2,4)
    {
        ...
        if keys[0] > keys[2] || (keys[0] == keys[2] && values[0] > values[2]) {
            // swap(0,2)
            {
                let tmp_572 = keys[0]; keys[0] = keys[2]; keys[2] = tmp_572;
                let tmp_573 = values[0]; values[0] = values[2]; values[2] = tmp_573;
            }
        }
        // cmp_swap(1,3)
        if keys[1] > keys[3] || (keys[1] == keys[3] && values[1] > values[3]) {
            // swap(1,3)
            {
                let tmp_574 = keys[1]; keys[1] = keys[3]; keys[3] = tmp_574;
                let tmp_575 = values[1]; values[1] = values[3]; values[3] = tmp_575;
            }
        }
    }
