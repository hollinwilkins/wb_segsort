...
# %% [markdown]
# ## CuteSort vs register bitonic (small-segment tier)
#
# **cute** is the ballot-multisplit register *radix rank* sort from GPUSorting
# (b0nes164) — each lane computes its sorted rank with `subgroupBallot` over the
# key bits, then a single smem scatter. It replaces the bitonic **reg** network
# on the small-segment tier, so it only exists where the whole segment fits in
# one subgroup (here N <= 32 on a 32-lane device). This is the direct
# apples-to-apples rival to **reg**: O(bits) ballots + one scatter vs reg's
# O(log^2 N) shuffle-network stages.
#
# Top: best-kernel throughput per N for each family. Bottom: cute/reg speedup
# (bars above 1.0 = cute wins).
...
# %% [markdown]
# ## cutemerge vs hybmerge (register baseline) — the merge tier
#
# **cutemerge** mirrors **hybmerge** (subgroup register run -> merge-path through
# smem) but sorts each subgroup's register run with the ballot-multisplit
# **CuteSort** instead of a bitonic network. Because CuteSort ranks one element
# per lane, a run of WPT keys/lane becomes WPT sorted SG-runs, so cutemerge does
# log2(WPT) extra merge passes. **reg** is the baseline (pure register bitonic,
# no smem merge) to anchor both against the plain small-segment strategy.
#
# Top: best-kernel throughput per N. Bottom: cutemerge/hybmerge speedup
# (bars above 1.0 = cutemerge wins).
